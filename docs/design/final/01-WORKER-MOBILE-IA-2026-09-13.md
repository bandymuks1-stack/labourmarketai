...
| Journal page (1,662 lines: composer, list, work-intelligence, gallery, corrections in one scroll) | `app/[locale]/dashboard/journal/page.tsx` | **REDESIGN** | Split into *Mano darbas* (record + list) and *Mano veikla skaičiais* (figures) — same readers, two destinations. The day list became a real calendar (2026-09-13 compaction, receipt `02-MOBILE-COMPACTION-RECEIPT-2026-09-13.md`). |
| Journal day-chip strip (`journal-day-nav`) | `app/[locale]/dashboard/journal/page.tsx` | **REPLACED** | A flat row of the days that already had records, over an endless list of date headings — it could not show the shape of a month and could not reach a day with nothing recorded yet. Replaced by `components/app/journal/journal-calendar.tsx` (month · week · day, URL state, same day groups). |
...
## 9. Not in scope, and the one constitution conflict
