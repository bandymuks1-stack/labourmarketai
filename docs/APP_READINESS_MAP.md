...
| `IOS_RUNTIME_JOURNEY_PROVEN` | **NOT YET** — token defined 2026-08-31 (`docs/mobile/NATIVE_READINESS_2026-08-29.md`): `ios.yml` now builds Release with the embedded Hermes bundle and runs a Maestro flow (`apps/mobile/.maestro/auth-failure-journey.yaml`) asserting the auth screen renders, a real sign-in attempt surfaces its failure (`testID="auth-failure"`), and the app stays interactive. Claimed only when that CI run is green. NOT claimed regardless: device, signing, store, real-backend session |
