...
Migration: `supabase/migrations/20260817121000_notification_document_types_v3.sql`
...
`worker_document`, `org_document`, `document_acknowledgement`. No RLS,
grant, index, column or row changes — the v1 posture (anon nothing;
recipient-only read + read_at mark; service-role-only writer; dedupe
UNIQUE) is untouched.
...
> `20260817121000_notification_document_types_v3` — exactly that one apply,
