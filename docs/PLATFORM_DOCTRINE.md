...
> **§2.4 Locale set (binding; amended 2026-06-12; amended 2026-07-04).** The canonical **taxonomy/recognition locale set is 12**: EN (source) + 9 launch markets (LT, LV, EE, NL, DE, DK, NO, SE, PL) + RU (worker language across markets — most workers on the first real sites operate primarily in Russian; RU is not a market of its own) + **FI (taxonomy + recognition locale, DI-approved amendment 2026-07-04 — Finnish worker-text recognition and taxonomy names; NOT yet a product UI route locale)**. All 11 full-UI JSON files must exist in the repo at all times, and the FI taxonomy directory (`messages/fi/`) must exist at all times (guarded by `lib/guards/skill-installation-chain.test.ts`). No PR may remove a locale. New i18n keys must be added to all 11 full-UI files in the same PR — placeholder values `[EN] <english>` are acceptable for non-Tier-1, non-ACTIVE locales until human or community translation lands; ACTIVE locales (lt, en, ru) must receive real translations in the same PR (guarded by `lib/guards/i18n-lt-en-parity.test.ts` and the `ru: 0` debt ratchet). New TAXONOMY keys (skill-names, professions, journal namespace, labour-market, productivity-units, relationship-types) land in all 12 taxonomy locales in the same PR. Translation completeness is tracked per-locale: Tier 1 = human-verified (EN, LT for M1); Tier 2 = baseline + moderation (the others, including RU — AI-seeded full translation pending human review per §7.4). FI is a taxonomy+recognition locale only until DI promotes it to the routing set (§2.5 one-row add) — FI must never be presented as a full product UI locale until that promotion. Tier promotion is independent of the locale set — the locale set never shrinks.

Implementation note (ISO 639-1 locale codes): full-UI set `en, lt, lv, et, nl, de, da, no, sv, pl, ru`; taxonomy/recognition set additionally includes `fi` (market → code: EE→et, DK→da, NO→no, SE→sv, FI→fi). The UI set lives in `apps/web/lib/i18n/config.ts` (routing + middleware derive from it; `fi` is deliberately NOT listed there); per-locale files are `messages/{locale}.json` plus the taxonomy layers `messages/{locale}/professions.json` and `messages/{locale}/skill-names.json`. Offline text recognition per language lives in `apps/web/lib/structuring/language-packs/` (owner offline mandate 2026-07-04).
...
|---|---|---|---|
| 2026-07-04 | §2.4 | Amend §2.4 to a 12-locale TAXONOMY/RECOGNITION set: add FI as taxonomy + recognition locale (DI approval 2026-07-04, offline-recognition train PRs #589/#593/#591/#592). FI ships `messages/fi/` (all six taxonomy files, EN key parity) and an offline recognition pack; the full-UI routing set stays 11 (`fi` deliberately NOT in `lib/i18n/config.ts`) — FI must never be presented as a full product UI locale until DI promotes it (§2.5 one-row add). Presence + parity guarded by `skill-installation-chain` / `universal-profession-families` / `journal-namespace-cvbridge` guards (12 locales). Offline recognition mandate recorded: recognition runs from bundled dictionaries only — no online APIs, cloud LLMs, remote translation, or live ESCO at runtime (guarded by `lib/guards/offline-language-pack.test.ts`). | DI + Claude Code |
