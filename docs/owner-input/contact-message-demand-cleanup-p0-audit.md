...
| Account dropdown **Bookings** link (`/dashboard/bookings`) | **CONSOLIDATED** — removed from dropdown; **intended home = `Žinutės`** (booking proposals / reservation requests are conversation/request objects) **+ a real `Mano erdvė` next-action card later, only when real pending-booking data exists**. No fake card/badge added now. Route stays valid (no dead link). **RED** until the data model wires a real pending count/source into Žinutės/dashboard. | `components/app/account-menu.tsx` (removal); home = `Žinutės` (future) |
