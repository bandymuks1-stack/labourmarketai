-- ============================================================================
-- PROPOSED (NOT a migration file — lives under docs/ on purpose). RED class:
-- replaces the body of a SECURITY DEFINER function. Route: owner review →
-- copy to supabase/migrations/YYYYMMDDHHMMSS_timesheet_entered_by_provenance_v2.sql
-- + paired supabase/rollbacks/<same>.down.sql, bump the migration-count
-- ratchet guards, apply ONLY via Supabase MCP apply_migration. Never db push.
--
-- timesheet_compute_lines_v1 v2 — allocation lines carry WHO recorded them.
--
-- WHAT CHANGES (three additive spots, derived from the applied 20260831170000
-- body with every other byte verbatim — generated, not hand-copied):
--   * alloc CTE          : + a.entered_by
--   * alloc_shaped CTE   : + a.entered_by, + (a.entered_by = w.profile_id) as
--                          entered_by_self, + left join public.workers w on
--                          w.id = p_worker_id (1:1 by primary key — cannot fan out)
--   * a_lines JSON       : + 'enteredBy' (uuid), + 'enteredBySelf' (boolean)
-- WHAT DOES NOT CHANGE: signature, LANGUAGE sql, SECURITY DEFINER,
-- SET search_path = public, STABLE, every REVOKE, totals, ordering, cap,
-- dedupe, tenant predicates, journal derivation. Journal-derived lines gain
-- no key (always the worker's own entries). A snapshot consumer that ignores
...
as $$
  ...
  alloc_shaped as (
    select a.id,
           ...
           a.work_object_id,
           a.entered_by,
           (a.entered_by = w.profile_id) as entered_by_self,
      ...
      from alloc a
      left join public.workers w on w.id = p_worker_id
  ...
  a_lines as (
    select al.work_date  as line_day,
           ...
           jsonb_build_object(
             ...
             -- PROVENANCE (window 6 §8): WHO recorded the hours. entered_by is
             -- the acting profile (RLS-enforced = auth.uid() at write time);
             -- enteredBySelf is true when that profile is the worker's own.
             -- Journal-derived lines carry neither key: they are always the
             -- worker's own entries.
             'enteredBy',      al.entered_by,
             'enteredBySelf',  al.entered_by_self
           ) as line
...
-- Privileges of a `create or replace`d function are preserved; the REVOKEs
-- from 20260817170000 (re-asserted 20260818150000 / 20260819220000 /
-- 20260831170000) still stand. Re-asserted so no reader has to prove absence.
