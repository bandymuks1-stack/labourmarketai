...
> not exist). The visual set `01–27 + A1–A4` was NOT delivered with the handoff — packages are
> implemented from the textual contract + `00-GALUTINE-DIZAINO-SISTEMA.md`; tokens only from
> `apps/web/tokens/*` and `globals.css`.
...
|---|---|---|
| `walk-gemini-proposer-prod.cjs` | **PARTIAL** — the proposer IS reached in production (two paid `ai_runs` rows `propose_conversation_intent`, `gemini-3.5-flash-lite`, schema passed, ≈$0.001 each; "Pastolių montavimą užbaigėme" → `task-status` → the chat asked "Kuris projektas?" from real rows; "Ką dar turiu susitvarkyti… Vokietiją?" → `documents` → the document gap answer); **defect D1**: "kuriems mano objektams gresia problemos" and "kurie darbuotojai nebus užimti" were caught deterministically as `log-work` / `need-workers` so the proposer never ran — router precision, not the proposer | `ai_runs`, `pilot_events chat_intent_recognized` 08:45–08:48 UTC |
| `walk-confirm-work-prod.cjs` (§14) | **FAILED at step 1 — defect D2**: "ką reikia patvirtinti?" → "Laukiančių darbo įrašų nėra. Darbų peržiūra dar neįjungta: **#8cda64**" — no "Įjungti peržiūrą: E2E Worker Two" chip, a raw profile-id fragment shown to a human; readback: engagement `90da8c16` `journal_review_enabled=false`, 0 confirmations for entry `01d4a36d`, 0 verified skills | walk log + MCP readback |
| `walk-gap-resolution-prod.cjs` (§11/§12/§16) | **PARTIAL — defect D3**: manager side works (gap line 0/7 → "Paprašyti" → "Nurodymas išsiųstas" → "Gauta" → "Patikrinta" persisted); the person's side is silent in the CHAT (no "Laukia nurodymų" brief line, no asks line under "mano projektai", no "recorded" / reply offer after saving the prefilled document) while the instructions PAGE renders the same rows (parity broken chat-side); the person's "mano dokumentai" counts the new record | walk log; residue to clean (document row, readiness rows `received`/`checked` → `needed`, instruction message) |
