...
## 0. Mutable facts (verified 2026-09-05 14:30 UTC)

| Fact | Value |
|---|---|
| `main` | **`c893557b`** (#1535 QA fixes, #1536 P2/P6 provenance edge, #1538 P8 World subset, #1537 docs — all merged 13:3x–14:1x UTC) |
| Production | **`c893557b`** = `main` (served by 14:15 UTC). Walked 14:18–14:25 UTC: card provenance edge + chat object language, World discovery, company-home regression — PROVEN; one visual defect (390 px provenance clipping) → wrap fix PR |
| #1441 Stripe (RED, `needs-human-gate`, draft) | head **`6bca4efc`** (Checkout `automatic_tax` + address + VAT id; `rk_live_` accepted). **LIVE inventory 14:05 UTC via the owner's restricted key**: Stripe Tax ACTIVE (reused), product `prod_VCjq8iLCnyfbr2` CREATED, Customer Portal config `bpc_1UCKNU637uptAg5z3ztY0snr` CREATED (default), **€99 price NOT yet created (key lacks Prices write)**, webhook endpoint MISSING (owner). `plans` data step applied. Remaining owner steps in the checkpoint's G-8 bullet |
...
| **On `c893557b` (14:18–14:25 UTC)** | **P2/P6 card edge PROD_PROVEN** (B13 → proven, C12 → proven; defect: 390 px clipping → wrap fix) · **P8 World subset PROD_PROVEN** (H2 subset; full H2 stays PARTIAL for the 1M validation) · **chat object language PROVEN** (0 raw ids; DERIVED marker) · **#1535 regression clean** (2 attention lines, no unavailable/none) | `walk-provenance.log`, `walk-provenance-chat.log`, `walk-world.log` |

## 1. Finite stage table (real end-to-end capabilities; PRs/tests/docs are not stages)

States: `PROD_PROVEN` · `IMPL_PENDING_PROOF` · `PARTIAL` · `NOT_IMPLEMENTED` · `OWNER_BLOCKED` · `EXTERNAL_BLOCKED` · `EXTERNAL_REAL_CUSTOMER_PROOF_PENDING` (owner directive 2026-09-05: real-money legs are never exercised with the owner's own card; they close with the first genuine paying customer). "Design" = frozen package that touches the stage (F = functional only).
...
| B13 | Identity reflects confirmed result | worker | "Darbdavys patvirtino…" on the brief + gold edge on the card | PROD_PROVEN | brief line 09:12 UTC `169df06f`; **card edge on `c893557b` 14:20 UTC (#1536)**: "mano kortelė" → `worker-player-card[data-provenance=EMPLOYER_CONFIRMED]`, gold edge, line "Patvirtino E2E Walker UAB, 2026-09-05"; the same answer says "Darbdavys patvirtino 1 jūsų darbo įrašą per 7 d."; negative control (learner) → `SELF_DECLARED`, dashed edge (`walk-provenance.log`, `walk-provenance-chat.log`) | wrap fix #1539 **PROVEN on `517dc890` 14:55 UTC** (390 px: full text, 3 lines, not clipped — `walk-provenance-wrap-517dc890.log`); full P6 identity view = V1 | P6-subset | IDENTITY | yes |
...
| C12 | Resulting state propagates | both | brief, card, pulse, risk agree | PROD_PROVEN | on `c893557b`: brief line + card edge agree (EMPLOYER_CONFIRMED); readiness ratio 0/7 in the chat = the Field rows = the ledger; risk line "1 žmogui trūksta dokumentų" on the company home = the same rows | — (wrap fix proven on `517dc890`) | F | — | — |
...
| G2 | Gemini fallback (proposer) | all | unmatched sentence → existing intent → same handler | PROD_PROVEN | reached in prod (ai_runs 08:46, 10:44, 10:45); **D1 fixed and walked 10:43 UTC on `51cd84b2`** | risk handler should answer across projects when none named (CONV follow-up) | F | — | — |
...
| H2 | World map layers / viewport | org | bounded, clustered, ≤60 objects | PARTIAL | **P8 subset PROD_PROVEN on `c893557b` 14:22 UTC (#1538)**: `/lt/dashboard/market-map` → `market-map-world[data-world-layer=demand]`, 3 pills, `data-map-origin=live`, "Matoma: 2 vietos, 2 objektai · Mastelis: šalys", 60-place cap named, rows `data-provenance=derived` (country centroids), projects → `empty` (prod projects have no country), supply → `empty` + aggregate note, 0 write requests, 390 px overflow 0, anon → login (`walk-world.log`) | 1M-point load validation (V1); bbox RPC for worker demand; the page still stacks three map instances (P5 composition later) | P8 | WORLD | after P2 |
...
| J1 | Pricing contract €0 / €0 (1) / €99 (10) / >10 contact | org | `/pricing` + `plans` rows | PARTIAL | `plans` rows €0 / €99 / entitlement 1 / 10; #1441 served `f4b5e582`; the account shows the ORGANIZATION order button | **public `/pricing` shows no figure**: the anon role has no table GRANT on `plans` (RLS `plans_select` is `true`, privilege revoked by the secdef pass) → fix pending; hero/boundary copy still says prices not final / payments off (copy gate on `isStripeActive`) | P9 | BILLING (reserved → owner gate) | no writer |
| J2 | FREE=1 / PAID=10 / 11th → individual plan | org | open-needs gate on the ONE demand path | PROD_PROVEN (FREE=1) · PAID=10 pending | `f4b5e582` 17:07 UTC: E2E Walker UAB (1 active need) → 2nd need refused: "Šios organizacijos nemokamas planas leidžia 1 aktyvią poziciją. … aktyvuokite planą „Organizacija“ (99 €/mėn., iki 10)" (`walk-stripe-live-before.log`) | PAID=10 and the 11th-need individual path = unit-proven (#1441 `launch-pricing.test.ts`); in production EXTERNAL_REAL_CUSTOMER_PROOF_PENDING (no fabricated subscription) | P9 | BILLING | — |
| J3 | Checkout → signed webhook → subscription → entitlement | org | canonical `billing_subscriptions` | PARTIAL → checkout creation PROD_PROVEN | `f4b5e582`: billing state `stripe_live`; POST `/api/billing/test-checkout` → 200, LIVE Checkout Session `cs_live_a1f1…` at checkout.stripe.com: mode subscription, `price_1UCKgg637uptAg5zD8dMA6kU` × 1, 9900 EUR/month, automatic_tax enabled, tax id collection, address required, metadata plan_key/organization_id/owner_id, success/cancel URLs; Stripe customer `cus_VCmctzUiZ8NDw4` (live) mapped in `billing_customers`; 0 subscriptions / payment intents / charges; `billing_subscriptions` 0, `payment_webhook_events` 0 → entitlement ungranted | signed webhook → subscription → entitlement 10 = EXTERNAL_REAL_CUSTOMER_PROOF_PENDING (owner directive: no owner payment; Stripe live mode has no zero-impact test event). Metadata `environment=test` is a stale constant (`STRIPE_METADATA_ENVIRONMENT`) → billing follow-up | P9 | BILLING | — |
| J4 | Account state + portal | org | portal session | PARTIAL | Customer Portal configuration `bpc_1UCKNU637uptAg5z3ztY0snr` (default, cancel at period end, invoices, payment method, address/tax id) verified via the Stripe API; `/api/billing/portal` refuses honestly without a subscription | portal SESSION for a paying customer = EXTERNAL_REAL_CUSTOMER_PROOF_PENDING | P9 | BILLING | — |
| J5 | Cancellation / refund readback | org | entitlement follows state | EXTERNAL_REAL_CUSTOMER_PROOF_PENDING | — | cancellation/refund readback follows the first genuine paying customer (owner directive 2026-09-05: no artificial turnover) | P9 | BILLING | — |
...
| K4 | Webhook security / secrets | org | signature, idempotency, mode match | PARTIAL | signature verification + idempotency + mode match unit/integration-proven with the real Stripe SDK signing (`webhook-signature.integration.test.ts`, `webhook-core.test.ts`, `webhook-route.test.ts`, 187 billing tests on `f4b5e582`); LIVE endpoint exists with the 10 events; secret in Production | first LIVE event = EXTERNAL_REAL_CUSTOMER_PROOF_PENDING | P9 | BILLING | — |
...
| L1 | No architecture vocabulary | all | ordinary words only | PARTIAL | `#8cda64` fixed; on `c893557b` "kas laisvas?" and the readiness answer carry 0 raw id fragments; readiness ends "Išvesta iš dokumentų sąrašo ir profilio įrašų — ne įvertinimas." | chip vocabulary findings (I2); `market-drilldown.tsx` shows "Projekto ID" (WORLD/workspace owner) | P2 | OBJ-UI + CONV | after J |
...
**COMMERCIAL_CRITICAL_STAGES (30):** A1 A2 B1 B3 B4 B5 B6 B7 B8 B9 B12 B13 C1 C2 C3 C4 C5 C6 C12 G1 G2 J1 J2 J3 J4 J5 K2 K4 L2 L3 — **proven 24 / 30 at 14:30 UTC 2026-09-05** (B13 + C12 proven on `c893557b`; previously (13:08–13:15 UTC on `08629b19`: A1, B6 re-walk, C6 Field UI, L2 company home moved to proven; earlier today B7, B12, B13, B8, G2). SAFE PILOT: **31 / 33**. Remaining in the 30: J1–J5 + K4 only (#1441, owner gate G-8 + real payment). IMPLEMENTED_PENDING_PROD_PROOF outside the 30: E5 (outcomes walk).
...
## 7. LANE RECOVERY COORDINATES (5h checkpoint, 2026-09-05 ~10:55 UTC)

All worktrees under `C:\Users\Mano\Documents\labourmarketai-wt\`. Base for every lane below = `origin/main` at the SHA named. No agent is running; no local watcher; CI/Vercel continue externally.

| Lane | Domain | Worktree · branch | Base → head | PR / state | Complete | Remains | Tests | CI | Prod proof | Next exact action |
|---|---|---|---|---|---|---|---|---|---|---|
| J invitation | CONV | `lane-j-invitations` · `feat/cc/invitation-attention-journey` | `169df06f` → `a4cfff30` | #1522 MERGED `dec7877d` | journey | decline-by-id (migration, owner); e-mail env | local + CI green | green | **PROVEN `59b94523`** | none; QA P3 (fingerprint by row) → next CONV batch |
| D2 name | company | `master-orch` · `fix/cc/confirm-work-person-name` | `169df06f` → `2c873b19` | #1523 MERGED `d292e998` | fix | — | 32 + negative control | green | **PROVEN `59b94523`** | none |
| D1 router | CONV | `lane-j-invitations` · `fix/cc/router-precision-d1` | `dec7877d` → `6b040c15` | #1526 MERGED `c1275c7a` | fix + 448 pins | risk handler cross-project answer; `laisvai` negative pin | 448/448 | green | **PROVEN `51cd84b2`** | none |
| P3 ledger | READINESS | `lane-p3-readiness` · `feat/cc/design-p3-requirement-ledger` | `169df06f` → `4a94088b` | #1525 MERGED `ea47d022` | read model + type + instructions page consumer | chat wiring (CONV); covering indexes (GREEN migration) | 30 | green | **PROVEN `51cd84b2`** | none |
| P3 QA fixes | READINESS | `master-orch` · `fix/cc/requirement-ledger-qa-notes` | `2490eab1` → `7aaa105e` | #1529 MERGED `332e1e30` | F1 country validation, F6, F3, F4 | F2 indexes | 63 | green | pending deploy | re-walk ledger once served (same walk) |
| P1 public entry | PUBLIC | `lane-p1-public-entry` · `feat/cc/design-p1-public-entry` | `169df06f` → `0f1e581f` | #1528 MERGED `2490eab1` | entry → router → doors → `?next=/dashboard?say=` → `landing_intent` | 5 untracked e2e screenshots in `docs/audits/evidence/**` (delete) | local + CI green | green | pending deploy | walk the 5 sentences on `/lt` + readback |
| `?say=` consumer | CONV | `master-orch` · `feat/cc/chat-consumes-say` | `332e1e30` → `5c77494b` | **#1531 OPEN, auto-merge armed** | hook + guard | — | 36 | running | — | after merge+deploy: signup door → dashboard → first turn = sentence |
| P4 Field subset | PROJECT UI | `lane-p4-field` · `feat/cc/design-p4-field-subset` | `2490eab1` → `78681a1b` | **#1530 OPEN, auto-merge armed** | Field scene + list + context, same actions | drag/what-if (P4 full), assign-from-slot UI | 27 + guards | running | — | walk: operations → token → Mark received → readback → Still needed |
| P5/C1 subset | company home | `lane-p5-employer` · `feat/cc/design-p5-c1-employer-subset` | `2490eab1` → `7827e962` | **#1532 OPEN, auto-merge armed** | capabilities strip, projects-in-time rows, who is free, missing in 4 weeks, needs-you, partners | 390 px visual proof on prod | 13 + guards | running | — | walk without the composer (8 steps in the PR body) |
| P2/P6 subset | CONV + IDENTITY | `lane-p3-readiness` · `feat/cc/design-p2-p6-object-language` | `c1275c7a` → **`9a25bdb6` WIP** | no PR (wip commit, pushed) | `lib/evidence/provenance.ts` + test, `components/app/provenance/provenance-edge.tsx` | provenance on the canonical card, chat render places, fact/derived marking, guard | unverified | — | — | resume the writer from the branch with the original brief (map §4 lane R) |
| P8 World subset | WORLD | `lane-p8-world` · `feat/cc/design-p8-world-subset` | `2490eab1` (clean) | none | nothing yet | all | — | — | — | launch the writer with the P8 brief (no new RPC; bbox filters on existing columns) |
| S billing | BILLING | main checkout · `feat/cc/stripe-live-activation-draft` | `332e1e30`? (rebased onto `997443df`+) → `d2bdae56` | #1441 RED draft, MERGEABLE | all code | owner G-8 + `plans` data step + real payment + refund walk | 266 billing tests | green at previous head | none (0 live rows) | owner Stripe session (map §6 block) |
| QA | read-only | — | — | #1522 APPROVE (1×P3); #1525/#1526 APPROVE (F1–F6 → #1529) | — | review #1530/#1531/#1532 after merge | — | — | — | run the read-only review on the three merges |
| Docs | docs/launch | `lane-docs` · this branch | — | this PR | checkpoint + map + walks/logs preserved | — | — | — | — | — |

Residue at checkpoint: ZERO from today's walks (invitation row, instruction messages, document rows deleted; readiness rows all `needed`; the engagement `90da8c16` review flag is ON with 1 kept confirmation on `01d4a36d` = evidence). Stripe agent leftover `PRODUCT_ARCHITECTURE_DIFF.md` is in `git stash` of the main checkout ("stripe-agent-leftover").
