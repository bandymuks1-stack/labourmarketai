...
|---|---|---|---|---|---|---|---|
| 1 | **G-1** | Provider dashboard → add domain `labourmarket.ai` (EU) → 3 DNS records; Supabase → Authentication → Emails → SMTP: host `smtp.resend.com`, port `465`, user `resend`, password = API key, sender `noreply@labourmarket.ai` / `LabourMarket.ai`; Rate limits → e-mails/hour `100`; Email → OTP expiry `86400`; template link per [`email-delivery-gate.md`](../human-gates/email-delivery-gate.md) Step 3; Vercel env `INVITE_EMAIL_PROVIDER=resend`, `INVITE_EMAIL_API_KEY`, `INVITE_EMAIL_FROM` | Provider **Resend** (decided from evidence; Postmark equal) | 20 min | €0 at launch volume | yes | agent runs the expiry / resend / failure proofs; the owner does ONE real-inbox registration on a phone (1 min) → G-1 closed → Train A complete |
