...
| **D** payments | canonical price set → live Stripe under RED gate → full chain E2E | **G-7 CLOSED 2026-09-05 (owner approval + same-day correction): PERSON €0 · ORGANIZATION FREE €0 (1 active position) · ORGANIZATION €99/month (up to 10) · >10 = individual plan (contact), everything else deferred.** BUILT on #1441 (rebased onto main): `PRICING_READINESS_STATE=owner_confirmed`; the figure only in `plans.price_eur_monthly` (prod rows updated: free 0 / business 99 / agency+enterprise inactive); registry `free_organization` (1) + `company_pilot`=ORGANIZATION (10, the only sellable plan) + deferred `worker_plus`/`agency_pilot`; the open-needs seam on the ONE demand path (FREE 1 / 10 / above → individual plan, chat + visual form; permissive until a Stripe adapter state is active); `/pricing` €0 / €99 / "Need more?"; the order button in the organization account (org-bound checkout); tests green. **D3 PENDING = G-8 owner actions** ([gate doc](../human-gates/payments-price-table-gate.md)): Stripe LIVE product €99, Stripe Tax, webhook endpoint + secret, Customer Portal config, 8 Vercel production variables incl. `STRIPE_LIVE_ACTIVATION=approved-by-owner`, approve + merge #1441; then ONE production acceptance (`walk-stripe-live-prod.cjs` before/after the owner's smallest real payment + refund). Production billing chain still NOT PROVEN (0 rows). |
...
- **2026-09-05 18:49 UTC — BILLING SAFETY v1 (RED, billing) APPLIED TO PROD** after the owner sentence "apply billing safety 2026-09-05" (chat). Ledger `20260905184921 billing_safety_invariants_v1` = repo `supabase/migrations/20260905200000_billing_safety_invariants_v1.sql` (rollback `supabase/rollbacks/20260905200000_billing_safety_invariants_v1.down.sql`, row-count-guarded). Readback (MCP): `billing_subscriptions` + `last_event_id`, `last_event_created_at`, `provider_price_id`, `unit_amount_cents`, `currency`; `payment_webhook_events.event_created_at` + index; `billing_customers` unique key now `(owner_id, provider, test_mode)` (old `(owner_id, provider)` dropped; 1 row untouched); new `billing_checkout_operations` — RLS ON, SELECT policy `is_admin()` for `authenticated`, grants `authenticated` SELECT / `service_role` SELECT+INSERT+UPDATE, **no anon grant**, 0 rows; partial unique indexes `open_scope_uniq` + `session_uniq`. **#1552 merged `ad50abe0`** (CI fully green). Migration count 265. Real settlement stays EXTERNAL_REAL_CUSTOMER_PROOF_PENDING (no owner payment). The reconcile route is superadmin-only and the only production admin is a real person's account — the agent does not impersonate it; the 200 report is one owner click on `/api/billing/reconcile`.

- **2026-09-03 evening — pilot-readiness run (owner mandate: IMPLEMENT → TEST → DEPLOY → PROD VERIFY).** Walked Lane A and Lane B as a first-time external user: code walk by three read-only agents (empty states per actor, opportunity-type pipeline, TTFV/social/G-1 chains) plus an **authenticated walk on the local stack** (moved to ports 553xx because Windows reserves 54290–54489; fixture institution "Dev Construction" with `training_provider`, fixture student with a seeded accepted invitation + current education row). **P0 — #1460 merged, production `cba98045`**: `/lt/signup` (the URL in the runbooks, the owner queue and the G-1 script) **returned 404 on production** at mobile width → `/:locale/{signup,login,register}` now 308 to `/auth/…` (**PROD VERIFIED**: `/lt/signup` → "Sukurkite paskyrą", `/lt/login` → "Prisijungti", `/en/register` → "Create your account"); institution "Invite learners" deep-links the panel with type + org + `relationship=student` pre-selected (was a collapsed panel four steps from the student option); programme form opens when there is no programme, "no learners yet" links to the invite, `unavailable` states say what happened; setup form offers the workspace and reports an unrecorded education preset (`capabilityDeclared`, never faked); agencies card hidden when nothing to act on; student quick-nav chip to the Learning Compass + compass steps are links + student intent lands on `#learning-compass`; board "being prepared" gets the profile CTA; roster card loses the "Empty state" badge and gets the agency variant; agency bridge roster link + `invalid`/`not-found` surfaced; client accept/decline/revoke errors surfaced; scouting empty state always links to demand intake; LT "Žvalgyba" → "Kandidatų atranka", raw "(scouting)" removed, tu → Jūs. **Local browser proof**: programme → cohort → learner assigned as the institution; invite deep link opens with "Join my company / organisation" · Dev Construction · "Studentas"; student sees the compass chip, the cohort block and linked steps. **P2 — #1461 merged (`b6514087`)**: employers can DECLARE the opportunity type in the normal wizard (it was buried under a collapsed "advanced" cluster — why 0 of 17 prod demands declare one); drafts keep it (whitelist + prefill re-validation); chat `company.create-demand` and the agent `demand.create` carry it as `structuredV2`; the employer's own request card shows it back; labels in the six locales that lacked them. **P4 — #1462 (auto-merge)**: TTFV now reports SYSTEM result (match preview; a board view with `candidate_count` > 0) and HUMAN response (contact disclosed, booking accepted, engagement created) separately; actor bucketing fixed (newest-first `onboarding_completed` with only `role_context` used to lock every student into "worker" and every institution into "employer"); `onboarding_completed` carries `intent`. **Correction of an earlier register claim**: `first_real_action` / `first_real_result` are declared and read by the aggregator but **never emitted** anywhere — the proxies stand in (listed on the admin page); #1447 shipped the intent telemetry only. **P6**: the login legal notice was Google-only copy; now provider-neutral in lt/en/ru/nl/de (the "redirecting" label was already neutral; buttons, callback, `next` sanitisation and the E2E honesty assertion are provider-generic — after "LinkedIn on"/"Facebook on" nothing on this side needs a deploy). **P7**: signup → e-mail → callback (PKCE + `token_hash`/`verifyOtp` cross-device fallback) → intent → destination verified by code walk; ready for `work`/`hire`/`agency`; education preset failure is now surfaced; the `token_hash` template switch stays in `docs/human-gates/email-delivery-gate.md`. **P8**: signup, login and landing render cleanly at 375×812 on production. Not done tonight (recorded): `/dashboard/start/*` hardcoded `label(lt, en)` strings (ru/nl/de see English on the first post-signup screen); in-memory wizard/invite state; no UI caller for `institution_learner_outcomes_v1`; agency-/institution-specific result events; public-intake and imported-ad type vocabularies (migrations). No real recruiter or institution used production tonight — both milestones stay FALSE.

- **2026-09-03 13:04 UTC — RED batch D APPLIED TO PROD** after the owner sentence "Apply batch 2026-09-03 D". Ledger `20260903130400 education_rls_recursion_fix_v1` (rollback: `supabase/rollbacks/20260903150000_education_rls_recursion_fix_v1.down.sql`). Readback on production: the three SELECT policies now evaluate through `is_active_cohort_member_v1` / `is_active_program_member_v1` / `education_cohort_organization_v1` (SECURITY DEFINER, `search_path=public`, EXECUTE anon = false, authenticated = true); `authenticated` still holds no INSERT/UPDATE/DELETE on the three tables; anon-executable definer set unchanged (8); rows 0/0/0. Per-actor SELECTs (rolled-back probe): manager 0/0/0 → 1/1/1 after creating a programme/cohort/membership through the definer commands; learner reads exactly its own membership joined to cohort + programme (1/1/1); outsider 0/0/0; learner direct INSERT 42501; anon 42501 on tables and helpers. Residue 0. The 42P17 recursion is gone: Lane B steps B2/B3/B6 are reachable again now (the fix is in the database; the UI needed no change). **#1457 merged → `13c2bc1e`** (migration count 262); its Vercel deployment was rate-limited, harmless for a DB-only PR. **Production = `953c34e7`** (#1459 chip) verified via `/api/health` 13:12 UTC. Next: walk the programmes section with the owner's real institution.

- **2026-09-03 window 2 — pilot walks against production found one real break; student cohort view shipped; Lane A chain proven.** Walked Lane B as the pilot would (reading as the institution manager and as a learner on production, rolled back): **every authenticated read of the three batch-B education tables fails with `42P17 infinite recursion detected in policy`** — the three SELECT policies reference each other; manager, learner and outsider alike; the readbacks after apply (ACLs, RLS on, 0 rows) were true and missed it because nothing SELECTed as `authenticated`. Effect: the institution programmes section renders "—" and never offers the create form (B2/B3/B6 unreachable in the UI); definer writes still work; no data wrong. **#1457 RED batch D** (draft, `needs-human-gate`, owner queue **row 0**): same readers, membership evaluated through three SECURITY DEFINER helpers (authenticated-only EXECUTE) — **proven on prod inside a rolled-back transaction** (manager 1/1/1, learner exactly own membership → cohort → programme, outsider 0/0/0, learner direct INSERT 42501, learner calling the manager command `not_manager`, anon 42501) and forward/down/forward on the local stack; `migration-safety` STRUCTURAL-GREEN with 3 acknowledged findings; ratchets 261 → 262. **#1458 merged** (`d5172597`): student sees own programme · cohort · institution · direction · dates and the SAME live demand count the institution sees, inside the Learning Compass; the programme's direction drives "what is missing" when the person has no own direction (registry, named as the programme's); no migration; lt/en/ru/nl/de; read path proven on prod (rolled back); degrades to "no cohort shown" until D is applied. **Lane A chain A4 → A8 PROD-PROVEN** (rolled back, zero residue): agency invites client → client accepts (JWT-email match) → client shares demand → agency sees it and offers a roster worker → agency cannot decide its own offer (`not_owner`) → client sees the offered candidate with the agency name and ACCEPTS → offer `accepted` + booking `proposed` for the right worker/request → agency progress `booking_started` → candidate sees the booking and accepts → engagement created, booking `accepted`, agency progress `accepted` → outsider sees 0 rows. Residue check: all probe tables back at 0. Prod events since 2026-09-02: no agency/education signups (0 rows in all four tables); the pilots still need real people. Harness: the local Supabase stack cannot bind its ports (Windows excluded range 54290–54389 → owner queue DEV-1); the 10 missing migrations were applied to the local DB via `docker exec psql` for the fix test. Deploy: `d5172597` deployed — health `build=d5172597` at 12:45 UTC. **#1459 merged** (`953c34e7`): the demand's DECLARED opportunity type (internship / apprenticeship / temporary assignment …) as a chip on board rows and compass fits — `JobRecommendation.opportunityType` from the structured projection, null when unstated, never inferred; labels reuse `structuredDemand.opportunityType.*`; unit- and type-verified only, because 0 of 17 prod demand rows declare a type (needs an employer internship posting, Lane B step B8). Vercel: `953c34e7` `pending` at merge; the docs-only push `7f15c221` was rate-limited (harmless — production already served `d5172597`); confirm `/api/health` `build` on resume. Track E: the Agentai capability contract lives in the agantai repo (`contracts/labourmarket-capability-contract.json`, last regenerated 2026-08-26) — cross-repo, not a labourmarketai slice.
...
---
| 2026-09-03 | **RED batch 2026-09-03 A+B+C APPLIED TO PROD** after the owner-requested final security review and the sentence "Apply batch 2026-09-03 A+B+C". Ledger: `20260903105726 public_vacancy_supply_counts_v1` (rollback: `supabase/rollbacks/20260903100000_public_vacancy_supply_counts_v1.down.sql`), `20260903105753 agency_candidate_offer_decision_v1` (rollback: `…101000_….down.sql`, guarded), `20260903105839 education_programs_cohorts_v1` (rollback: `…120000_….down.sql`, guarded), `20260903105902 institution_learner_outcomes_v1` (rollback: `…140000_….down.sql`). Review fixes before apply: A re-states `work_mem=64MB` (CREATE OR REPLACE drops SET config — prod-verified) + deterministic `not exists` fallback; B covering index (7.2 s → 176 ms) + `count_public_vacancies_by_profession_v1` authenticated-only; C suppression 3 → 5. Readback: cron `public-vacancy-supply-counts-10min` active, row 46487/8817, anon cannot read the counts table or call refresh; 4-value offer CHECK, 4 new columns, 0 offers; 3 education tables RLS on with SELECT-only policies, authenticated has no INSERT; anon-executable definer set unchanged (8). Health 200 (db 585 ms). | applied, verified by readback |
| 2026-09-03 | RED batch A/B/C PRs merged after apply: #1448 `6255e28e`, #1454 `1f6703fa` (programmes UI lost its needs-migration copy — batch applied, product-copy guard), #1456 `8d3e7dec`; migration count 261. **Production build is still `342d225b`: Vercel Hobby deploy quota exhausted at 10:51 UTC ("rate limited — retry in 24 hours"), every main push since fails to deploy — owner action VERCEL-1.** DB is ahead and additive; the running build is compatible. | merged; deploy blocked (owner) |
