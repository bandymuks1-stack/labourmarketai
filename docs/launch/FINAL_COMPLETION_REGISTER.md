...
|---|---|---|---|
| **A** security / auth / new-user / Connected Apps | confirm-email UX (cross-device link, `next` carry, resend, unconfirmed-login copy), Connected Apps surface, new-user browser walk, all 12 identity cases | auth PRODUCTION_PROVEN; new-user UI IMPLEMENTED_UNPROVEN; Connected Apps MISSING; delivery EXTERNAL_GATE G-1 | Playwright walk on prod for cases 5–8, 11, 12; Connected Apps list/revoke proven against the real ChatGPT grant |
| **B** DB lifecycle / capacity | expired-vacancy semantics in reads (non-destructive), thresholds doc, gate packs for A–D | reads still serve 25,635 expired ads as live: PARTIAL; gates OWNER_GATE G-3…G-6 | read-path fix proven; capacity thresholds recorded; gate packs with exact SQL + rollback |
| **C** social auth | LinkedIn + Facebook full matrix (signup, login, collision, linking, logout, re-login, provisioning, assistant continuation, mobile, cancel) | Google PRODUCTION_PROVEN; LI/FB EXTERNAL_GATE G-2 | per-provider matrix on prod with bounded identities |
...
| **E** worker / Living CV / Journal / import | credential validity model, journal input paths, XLSX import proven on a real company file with rollback + receipt | XLSX pipeline IMPLEMENTED_UNPROVEN; validity model PARTIAL | import of an owner-supplied historical file on prod with receipt + rollback proof |
| **F** employer / company / project / timesheet | company control operational loop: project → object → assignment → calendar → work → journal → hours → timesheet → approval → report | PARTIAL (M3 closed; calendar no write path; zero prod usage) | one real company chain on prod, end to end |
...
| **H** marketplace / matching / engagement | reachability (M7/M8) + two-sided lifecycle to `engagement_contexts` | loop works, unreachable: PARTIAL | demand → match → contact → booking → engagement proven on prod through native nav |
| **I** visual / UX / design-system | screenshot audit of every actor × route on prod; IA + nav + component hierarchy decision; consolidation (M10–M12) | MISSING (no current audit) | before/after screenshot pack per actor; no legacy-pattern route left unclassified |
...
| **K** privacy / security / SEO / AEO | public-jobs privacy rule audited across HTML/JSON/schema.org/API/metadata/cache; secret scanning; rate limits; MFA/passkey evaluation | PARTIAL | leak matrix with zero protected fields exposed; findings ≤ P2 |
| **L** observability / backup / scaling | error monitoring, health probe, critical-flow metrics, alerts tied to user impact, backup/rollback drill, thresholds for 100/1k/10k/100k | MISSING | alerts fire on a synthetic failure; rollback drill recorded |
...
| privacy controls | export/consent exist; full leak matrix not run |
