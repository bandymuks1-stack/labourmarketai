...
| G-12 | Apply RED draft **#1430** (K2-1 companies contact minimization: column grant + two definer readers; no data change; rollback restores the full grant) | OWNER_GATE | approve + apply via MCP | closes the open P1 |
...
| G-15 | Approve a one-line RPC change so `accept_company_worker_invitation` also binds the organisation membership (finding F4-1; RED by rule) | OWNER_GATE | approve the draft when opened | worker timesheets without a manual `add_org_member` |
| G-14 | Admin verification of a bounded test employer (`E2E Walker UAB`) so a demand can reach a worker board — the admin RPC refuses non-admins, even service_role | OWNER_GATE | one admin click / RPC call | Train H2 + Train M chain |
