...
| Item | Value |
|---|---|
| Production | **`fc579348`** (#1511 squash) 2026-09-05; read `https://labourmarket.ai/api/health` `.build` before ANY walk. The Vercel Hobby limit hit 22:57 UTC 2026-09-04 lasted hours, not 24 h — never push to test it |
| `main` | #1467–#1474 · #1476–#1509. **#1510 merged → production `f8db65b4` (04:47 UTC); the READINESS journey walk PASSED on it (ask → state → seed 7 rows → received → instruction → the person's brief).** #1511 landing honesty merged → production `fc579348` (04:5x UTC). Open next, ONE push: **`feat/cc/gemini-conversation-proposer`** (owner approval 2026-09-05 — task-scoped Gemini egress grant + the conversation intent proposer; docs: contract §1b scale rule, ARCHITECTURE §5.5 rows) → auto-merge → `walk-gemini-proposer-prod.cjs` (acceptance 1–8, PENDING_PROD_PROOF). RED draft: **#1475** My Space table (owner gate) |
...
2. DONE — #1511 merged (landing honesty + docs). Gemini proposer PR → merge → `EXPECT_BUILD=<sha> node walk-gemini-proposer-prod.cjs` → MCP readback (`ai_runs` task `propose_conversation_intent`; `pilot_events.metadata.resolution`) → record acceptance 1–8 in the night log.
