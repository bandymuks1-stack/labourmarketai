...
| Item | Value |
|---|---|
| Production | **`f8db65b4`** (#1510 squash) at 04:47 UTC 2026-09-05; read `https://labourmarket.ai/api/health` `.build` before ANY walk. The Vercel Hobby limit hit 22:57 UTC 2026-09-04 lasted hours, not 24 h — never push to test it |
| `main` | #1467–#1474 · #1476–#1509. **#1510 merged → production `f8db65b4` (04:47 UTC); the READINESS journey walk PASSED on it (ask → state → seed 7 rows → received → instruction → the person's brief).** Next single push: `fix/cc/landing-example-honesty` (landing never presents a scripted scenario as the visitor's answer; freeze baseline regenerated on record) + these docs. RED draft: **#1475** My Space table (owner gate) |
...
2. Rebase `fix/cc/landing-example-honesty` on main, add the night-log sections (readiness proof, Gemini findings), push ONCE → auto-merge → landing check on prod (a typed question shows `hero-own-question-note`).
3. Next journey batch: §14 CONFIRMATION — "patvirtink Jono darbą" / approval by sentence over the workflow engine (`requestTaskApprovalAction` + the approver's decision), readback on the CV sheet (employer-verified evidence).
...
- **Gemini for the conversation** → one dated, sourced row in `apps/web/lib/ai/runtime/data-egress.ts` `AI_EGRESS_GRANTS`: `{ provider: "gemini", maxSensitivity: "SENSITIVE_FREE_TEXT", basis: <DPA / terms reference>, grantedOn }` + confirmation the Gemini key is on a PAID tier. Until then the LLM proposal path is not built (it would be unreachable) and the chat stays deterministic.
