...
| Item | Value |
|---|---|
| Production | `a870427a` (#1467) verified ~09:05 UTC by the prod walk; starters mix + composed fallback PROVEN with the E2E agency identity (`pilot-feedback/2026-09-04-starters-not-a-role-menu.md`) |
| `main` | `a870427a` (#1467 starters are suggestions) — next: the employer-sentence PR (branch `feat/cc/employer-sentence-structure`) |
| Active P0 chain | **F — employer sentence → structured demand** (C+D DONE + prod-proven) |
...
| Employer | "reikia darbuotojų" → ONE inline form → demand → candidates → contact/booking | matching + shortlist (browser 2026-08-27); owner sentence structured (unit-proven, PR open) | prod walk of the owner sentence pending the F merge |
...
1. ~~C+D~~ DONE (#1467, prod-proven).
2. **F (employer sentence, PR open)**: five construction trades, `from_date` in six locales, city beside country, startDate → `structured_v2.time.start_earliest`, workType/country reach the canonical columns → after merge: prod walk with the E2E company identity (`walk-employer-sentence-prod.cjs`), then close the E2E demand.
