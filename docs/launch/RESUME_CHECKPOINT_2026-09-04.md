...
| Item | Value |
|---|---|
| Production | **`169df06f`** (#1518, Vercel deployed 08:43 UTC 2026-09-05 — the Hobby freeze lifted on the #1518 push; `fc579348` was served 05:04–08:43 UTC). Read `/api/health` `.build` before ANY walk; never push to test the limit |
...
| Active lanes (one write owner per domain) | **J** `feat/cc/invitation-attention-journey` (worktree `labourmarketai-wt/lane-j-invitations`, WIP `72e3602b` rebased on main; invitation → Attention → accept/decline over the dispatcher) · **P1** `feat/cc/design-p1-public-entry` (worktree `lane-p1-public-entry`; the public entry understands a real sentence) · **D** `feat/cc/master-completion-2026-09-05` (worktree `master-orch`; this checkpoint + the map) · **Q** read-only root cause of D1–D3 · **S** none (owner gate). The main checkout `Documents/labourmarketai` is left on the Stripe branch — no lane writes there |
...
1. RUNNING — lane J (invitation journey, 7 guard failures to fix, then PR + auto-merge) and lane P1 (public entry). Root-cause report for D1–D3 → the CONV fix batch is the NEXT item for the conversation domain once J's PR is up (same writer domain, serialized): D2 enable-review chip + person name in the confirm-work handler; D3 the person's chat-side reads (brief "Laukia nurodymų", asks under "mano projektai", recorded + reply offer); D1 router precision for "objektams gresia problemos" / "darbuotojai nebus užimti" and low-weight matches → the proposer.
