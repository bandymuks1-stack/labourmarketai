...
| APPLIED | **"Apply Lane A ownership 2026-09-04" given and executed**: migration on prod (ledger `20260904055214`), grant applied (admin membership, 0 companies created), per-actor readback green (real owns/roster 1, creator unchanged, outsider 0, anon 42501); #1464 ready + auto-merge; #1465 merged (`2039a197`). Real account = admin of "Labour market ai Sp. z o.o", onboarding closed by the grant (no `onboarding_completed` event — honest) |
| Chat-first P0 (06:15 UTC, real user) | the real recruiter typed "noriu pakviesti klientą" and got the WORKER fallback + employer chips. Root cause in the shared path (no agency vocabulary in the router; one worker fallback for all identities; `agency.*` gated on the legacy `agency` role). **#1466 merged → prod `00f3749a`**: 5 agency + 3 student/institution intents (5 locales), `agencyWorkspace` flag, context-aware fallback, `company.invite-worker`, agency bridge read adapter, readback, chat telemetry. Prod-proven with the E2E agency identity: sentence → one question → persisted pending connection → readback → telemetry chain. `docs/launch/pilot-feedback/2026-09-04-chat-first-agency.md` |
| Owner's next step | on the real account, type in the chat: **„noriu pakviesti klientą“**, answer the one question with the real client's e-mail, confirm. That persisted row is the first genuine recruiter product action; the agent takes over from it |
