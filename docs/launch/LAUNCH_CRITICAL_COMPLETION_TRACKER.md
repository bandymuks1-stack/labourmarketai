...
| `feat/org-demand-scope-app-gates-v1` (Stage A, in progress) | Workspace gates for 10 ungated employer paths + G6 divergent-resolver fix; `contact-interested-worker.ts` excluded (owned by #1015). |
| `feat/org-demand-spine-schema-v1` (Stage B, in progress) | Owner-gated org row-scope package being authored; prod preflight verified 2026-08-05: 17/17 + 1/1 + 0 rows org-resolvable = 100% backfill coverage, zero ambiguous. |
