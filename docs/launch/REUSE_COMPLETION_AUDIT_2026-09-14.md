...
| `lib/agency/pool.ts` (S5 agency worker pool) | `getAgencyPool()` and `markCanOfferAction` have **no caller anywhere**; the only "pool" on a page is the labelled marketing preview from `content/placeholders.ts`. Both RPCs (`list_open_demand_for_agencies`, `mark_agency_can_offer`) verified **present in production** 2026-09-14 | **BUILT_NOT_CONNECTED — and the decision has since been made.** A complete read service (worker cards, country readiness, open demand) with live RPCs and no surface. Left unmounted here because the two-agency-models question was open; **#1740 answered it on 2026-09-14** — Model B (the agency↔client bridge) is canonical and this Model A pool is retired as **ORG-10**, with `lib/guards/agency-model-b-canonical-v1.test.ts` banning its import from any route or component. Correctly not wired; no longer an open question |
...
| `withdraw_contact_disclosure_request_v1` | **Real gap — CLOSED by #1740** (`9af40cac`, "the employer can take back a contact request they sent"). Recorded as found, because the reasoning still explains the shape of the defect. At the time: an employer could not retract a pending ask for a worker's contact details; it sat in the worker's list until they answered or it expired after 14 days. Wiring it needed an employer-side read of their own outgoing asks, and none existed — `list_my_contact_disclosure_requests_v1` filtered `w.profile_id = auth.uid()`, i.e. the worker. That was a NEW privacy surface plus a NEW read, not a wire-up — which is why it was named rather than built here, and why #1740 is where it belonged |
