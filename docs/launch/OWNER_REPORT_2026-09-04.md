...
- Found tonight, recorded not fixed: on a PHONE the chips a panel action posts into the thread sit under the open bottom sheet (the person must collapse the sheet first); the employer's journal-review line needs the per-engagement review opt-in.
...
| PROJECT | create by sentence → structure (stages/dates in the panel) → people/team (assign) → work (journal with project context) → readiness (project centre) → progress → evidence → report (org figures by sentence; reports/evidence page) | P (late 2026-09-04): create by sentence → people (E2E Worker Two assigned by chip, row `80883119`) → work (the worker's chat log carries `project_id`, row `01d4a36d`) → evidence on the CV sheet; figures by sentence. S: employer REVIEW of that work needs `journal_review_enabled` on the engagement (a company-page opt-in, owner-hold v4 — not by sentence yet); —: Project Field / what-if movement (§11) |
