...
| PROJECT | create by sentence → structure (stages/dates in the panel) → people/team (assign) → work (journal with project context) → readiness (project centre) → progress → evidence → report (org figures by sentence; reports/evidence page) | P (late 2026-09-04): create by sentence → people (E2E Worker Two assigned by chip, row `80883119`) → work (the worker's chat log carries `project_id`, row `01d4a36d`) → evidence on the CV sheet; figures by sentence. G: employer REVIEW of that work — `journal_review_enabled` is an owner HOLD (v4): the RPCs reject enabling it, so the review line cannot appear until the owner lifts the hold; —: Project Field / what-if movement (§11) |
