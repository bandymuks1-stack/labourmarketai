...
1. **K2-1 v2 — company contact columns (replaces #1430).** Still needed: production ACL `authenticated=r` + `companies_select = auth.uid() IS NOT NULL` → every signed-in account reads every company's contact_email / contact_phone / address / registration_code / vat_number / requester_role / verification_note / requested_at. #1430 is STALE: since #1859 `readCompanyByIdForAccess` reads those columns for the creator AND every active ROLES_THAT_OPEN member; #1430's owner-only reader would answer 42501 on every company door and drop managers/members. v2 = discovery-column grant + ONE definer `read_companies_private_v1()` (creator, active owner/admin/manager/external_manager/member of the bound organization, admin) + the four app reads through `readCompaniesPrivate` (falls back while the function is absent). Reuses the existing membership model and `is_admin()`; every DB function that reads a private column is already a definer; no view depends on `companies`. Proven on a local PostgreSQL 16 with production's columns/policies/ACL: before = 8/8 actors read all contacts; after = direct private reads 42501 for all, discovery rows intact, reader returns exactly creator/manager/ext-manager/member → own, admin → all, stranger/revoked/invited → none, anon refused; rollback restores exactly; double apply idempotent. Order: merge + deploy the app half FIRST, then apply. Rollback: paired `.down.sql`. If not done: contact data of every company stays readable by any account.
2. **Manager RLS (no draft yet).** Measured: `projects_insert/update/delete` = `owns_company(company_id) OR is_admin()`, `projects_select` adds `is_assigned_to_project(id)`, `company_workers_select` = `owns_company OR owns_worker OR is_admin()`; `owns_company` admits the creator + ACTIVE owner/admin members only. The existing `manages_organization(org)` already admits active owner/admin/manager/external_manager. Proposal: `projects_select/insert/update` gain `OR manages_organization(organization_id)`, `company_workers_select` gains it via the organization bound to `company_id`; `projects_delete` unchanged unless the owner says otherwise. If not done: the Nonstop recruiter (manager) keeps getting 42501 on project writes and an empty workforce read (the app says so honestly).
3. **Company description save is broken (new defect, needs RED).** `saveOrganizationDescriptionAction` (`lib/company/description-actions.ts`, the public business page panel) does a direct `UPDATE companies SET description`, but `authenticated` holds NO UPDATE on `companies` (table or column) and `organizations` writes are admin-only → every save answers 42501 → generic error; 0 companies / 0 organizations carry a description on production. No existing definer writes `description` (`set_business_public_profile_v1` = slug/tagline/contact). Fix options: (a) `grant update (description) on companies to authenticated` — RLS `companies_update` then admits the creator + admin only (owner/admin MEMBERS still refused); (b) a definer `set_company_description_v1(company, text)` gated like `owns_company` (creator + active owner/admin member), matching the app's `manage-company-profile` capability. Recommended: (b).
