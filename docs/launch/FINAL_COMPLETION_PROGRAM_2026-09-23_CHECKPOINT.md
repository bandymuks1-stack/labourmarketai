...
## F. NONSTOP
- Consolidation applied and read back (PR #1847). Lane N (#1859) live on b237ff4f: the manager-role member opens the canonical workspace; members + invitations in Settings. Remaining: the director's and the recruiter's own switcher/workspace walk. RED manager RLS (projects select/insert/update + company_workers select → manages_organization) APPLIED 2026-09-24 (ledger 20260924092836, app half #1871) — DB-level proven; the recruiter's own walk (create a project, open the roster) is the UI proof still owed.
...
4. ~~RED manager RLS~~ APPLIED 2026-09-24 (§J #10c/#10d). Still open: membership_accept_v1 role grant vs the workspace-derived gate (#1859). The two #1871 follow-ups are settled by the owner direction in §J #10e (invitations: #1874 GREEN + #1875 RED draft; archived hardening withdrawn).
...
0. (#10e, 2026-09-24 ~11:40Z) OWNER DIRECTION — company permissions and archived organizations.
   - **(1) Invitations are managed by permission, never by job title.**
     ...
     - RED draft #1875, owner approval pending, NOT applied:
       - per-person delegation on `company_memberships`;
       - the organization-invitation authority becomes the owner, an owner/admin member or a delegated member;
       - one audited owner/admin grant command;
       - project and need invitations unchanged;
       - proven on a local PostgreSQL 16; the rollback restores production byte-identically.
     - The app half (grant toggle, reading the member's own flag, delegated organizations in the invite panel) follows the owner's approval.
   - **(2) Archived organizations: no authorization change.**
     - The database helpers never consult `archived_at`, so the owner and everyone entitled to a record keep their history. The app keeps archived organizations out of every workspace list, so they no longer operate.
     - The four archived organizations are the legacy Nonstop duplicate and three development/test shells from the 2026-09-23 consolidation. Their only history: the owners' own rows, default approval templates, one closed test need, and one person's journal entries under an ended engagement (still visible to that person).
     - The earlier "archived-organization authority hardening" RED item is WITHDRAWN: it contradicted the owner rule.
   - **Open for the owner:** the People page offers per-worker role controls to managers, and the server then refuses them. A correction was posted on #1871.
0. (#10d, 2026-09-24 ~10:00Z) #1871 MERGED `4ba60f7d` once CI was green on its head and every review thread was resolved → Vercel Production deployment of `4ba60f7d` = success (GitHub deployment 6635082807, 10:00:05Z). All three §K RED packets are now APPLIED: DB-level PRODUCTION-PROVEN, UI NOT PROVEN. The UI proofs need real signed-in sessions, and `labourmarket.ai` answers 403 from the cloud: (a) K2-1: owner AND manager each open their company page and see its details; (b) #1870: owner saves a description and sees it on the public page; (c) #1871: manager creates a project and opens the roster. Two #1871 follow-ups for the owner, neither started: (i) the pending-invitations list reads empty for a manager (pre-existing; roster management stays owner/admin; a small GREEN honesty fix in the UI, needs the owner's go); (ii) an organization-authority hardening item, RED with its own approval, with no effective access in production today (detail in the owner channel). Historical stays BLOCKED on the owner-PC batch-5 worktrees (#10); returning-user ŠIANDIEN: no change.
0. (#10c, 2026-09-24 ~09:35Z) Owner confirmed the RED review of #1870 / #1871 (diffs unchanged since approval). #1870 description save: MERGED `b17014a0` → Vercel Production success → rolled-back prod dry run → APPLIED (ledger `20260924092646`) → read-back; DB-level PRODUCTION-PROVEN, UI NOT PROVEN (owner saves a description, sees it on the public page). #1871 manager RLS: rolled-back prod dry run → APPLIED (ledger `20260924092836`) → read-back → app half merged via #1871; DB-level PRODUCTION-PROVEN, UI NOT PROVEN (manager creates a project and opens the roster). The only production manager is also a platform admin, so the manager arm was proven with a temporary non-admin manager seeded inside rolled-back transactions. #1872 (ledger evidence kept private; K2-1 app path open) MERGED `c96ea8b7`.
...
**RED packets (one decision each; nothing applied at checkpoint #10).** Status since: packets 1–3 all owner-approved and APPLIED on 2026-09-24 — see §J #10b, #10c, #10d.
