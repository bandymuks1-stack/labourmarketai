...
| **R-B** | Rewrite `search_public_vacancy_previews_v1` so the profession-filtered board stops timing out. | Cold plan reads ~14.9k pages for 20 welders (generic plan cannot use the profession index behind `p_profession_slug is null OR …`); 8.3 s under `anon` vs 3 s timeout; measured through the shipped function. SECDEF replace = RED (rule g), same class as #1572. | Same signature, same ACL, same NULL projection; body branches on the parameter (plpgsql `IF` or two mutually exclusive branches) so each path uses its index. Dry-run proof in a rolled-back transaction before apply. |
...
_Filled after merge and deploy — see the section appended below._
