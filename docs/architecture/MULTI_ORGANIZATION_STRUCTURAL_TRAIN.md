...
| M-P0-6 | Organization demand stamping v2 (supersedes #1016) | `ORGANIZATION_DEMAND_SPINE_V2_MULTI_ORG_CODE_COMPLETE_PENDING_HUMAN_GATE` | **Package complete (this branch)**: migration `20260806200000` (organization_id on demand/shortlist/booking + validated-workspace v2 entry RPCs + inheritance triggers + immutability guard + membership read legs + counted single-org backfill) — RED/UNAPPLIED, gate doc `docs/human-gates/org-demand-spine-v2-gate.md`; archive-not-drop rollback; app calls v2 with the workspace org, honest v1 fallback. Two-organization actor proof **14/14 PASS** (`docs/audits/evidence/multi-org-m-p0-6/`). #1016 closed as superseded |
| M-P0-7 | Billing subject model (`profile \| organization`) | `STRIPE_MULTI_SUBJECT_TEST_MODE_MODEL_CODE_COMPLETE_PENDING_OWNER_GATES` | **Model complete (this branch)**: canonical subject doc `docs/architecture/BILLING_SUBJECT_MODEL_V1.md`; `billing-subject.ts` (workspace proposes, `manage-billing` capability decides — owner/admin only); per-subject entitlement read in `effective-entitlements.ts` (org rows only / personal rows only, 42703 feature-detected — byte-identical behaviour until the schema applies). Schema half = owner-gated PR #844 migration (partial uniques + origin_organization_id); gates: apply #844 migration, Stripe TEST credentials, rebase #844's binding onto company_memberships |
| M-P0-8 | Organization-aware analytics attribution | `MULTI_ORG_ANALYTICS_ATTRIBUTION_PROVEN` | **Attribution seam live (this branch)**: `analytics-attribution.ts` (ONE resolver off the M-P0-7 billing subject — workspace type / org id / role / billing subject); server-funnel stamps every emitted event (13 sites inherit, no fabricated org possible); allowlist + FunnelMetadata gained the reserved keys (`workspace_type`/`organization_id`/`org_role`/`billing_subject`/`ref_type`/`ref_id`); AI cost writer resolves the workspace org (hardcoded null closed; personal/cron stay honest-null). §14 behaviour proof unit-level (A-only/B-only/personal/fail-safe) + structural pins (append-only history, PII-safe admin readers) |
