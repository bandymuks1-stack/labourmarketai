...
- **`20260807120000_admin_grant_service_role_repair_v1.sql` — founder admin-grant path repair (2026-08-06 session: `pnpm admin:promote` / `pnpm admin:grant-superadmin --apply` fail against production with "permission denied for table profiles"). HUMAN GATE: do NOT apply without explicit owner OK.** Column grants to **service_role ONLY**: `profiles` SELECT (id, email, active_role) + UPDATE (active_role); `profile_roles` SELECT/INSERT/UPDATE (profile_id, role). Zero DML, no new object, anon/authenticated NOT widened, no policy/trigger/RLS change. DIAGNOSIS CORRECTION recorded in the migration header: no hardening migration ever revoked these grants — service_role NEVER held a write privilege on `profiles`/`profile_roles` (the MCP-apply no-default-privileges pattern proven by 20260610234500 and 20260702200000), so the documented founder path (lib/auth/superadmin.ts header) could not have worked as privileged. DECISION: narrow column grants, NOT a SECURITY DEFINER admin-grant RPC — an RPC "gated on an existing admin" fails the bootstrap case (zero admins) and its break-glass arm collapses into these same grants plus new SECDEF audit surface; protection stays the APPLIED 20260702130000 admin-grant-guard triggers (service_role carries no JWT and passes by that migration's own design; JWT-carrying non-admins still blocked). Guard: `apps/web/lib/guards/admin-grant-service-role-repair-migration.test.ts`. Paired rollback: `supabase/rollbacks/20260807120000_admin_grant_service_role_repair_v1.down.sql` (revokes ONLY this migration's additions; the 20260702200000 read grants stay). NO `@human-gate-approved` — migration-safety stays RED by design; ships UNAPPLIED.
