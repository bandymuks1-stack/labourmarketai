...
- **`20260817153000_notification_events_v4_task_types.sql` — notification types v4: `work_task_assigned` + entity `work_task` (train D). PENDING APPLY BY LEAD — RECOUNT the constraint lists against the sibling v3 widenings (workflow/document types, PRs #1170/#1169) before applying; whichever widening applies LAST must carry the FULL accumulated list.** GREEN-idiom constraint drop+re-add (strict superset, the 20260813100000 v2 precedent) on `event_type` and `entity_type`. Emitter `emitWorkTaskAssignedNotification` resolves the recipient from the task row, never emits for self-assignment; href = existing /dashboard/tasks. Paired rollback (states its cost like the v2 rollback): `supabase/rollbacks/20260817153000_notification_events_v4_task_types.down.sql`. *Added 2026-08-17.*
