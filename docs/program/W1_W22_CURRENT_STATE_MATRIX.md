...
| W7 | Employee journey | ~68%; zero P0; P1-1/#981, P1-2/#979 closed; **P1-4 inventory DONE**; **W7-S1 SHIPPED** | #979 #981 #1051 | none own | local journey ✔ (83 shots) + profile measured at 1440/375 (`docs/audits/W7_PROFILE_FULL_INVENTORY.md`) + **W7-S1 BEFORE/AFTER browser proof, 3 identities × 2 breakpoints, 21/21 interaction checks** (`docs/audits/W7_S1_PROFILE_HUB_OVERVIEW.md`) | **W7-S1 DONE** — five duplicate readiness summaries consolidated into ONE `ProfileHubOverview`; page height −13.6%/−15.2% (partial worker), −19.0%/−19.1% (complete), −12.0%/−14.3% (company); cards 19→16; h2 14→12; sub-44px 75→71; overview moved from the 4th block to the 1st (y=1644→259). PROFILE simplicity 1.0→1.86 (PILOT_BLOCKER→NEEDS_POLISH), same rubric. REMAINING: W7-S2 a11y (34 unlabelled inputs, 70 sub-44px targets, all in `worker-availability-prefs-form`); W7-S3 ≈15 serial await stages still on the page; W7-S4 two misplaced sections (`#managed-companies`, `MessageButton`); W7-S5 non-worker identity copy; P1-3 conversation memory (SQL still in `docs/proposals/`, never a migration); P2-1 open-ended bookings skip the overlap guard | **PARTIAL** | **yes** | W7-S3 — flatten the page's remaining waterfall (behaviour-identical, no migration, no browser judgement needed) |
