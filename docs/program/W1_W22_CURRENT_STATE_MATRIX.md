...
| W6 | Trust / experience domain | **Schema ACTIVE in prod** (applied 2026-08-04, ledger 173→174); `experiences` result = `real`; local full cycle proven | #972–#974 #977 #1008 | `20260802120000` APPLIED | local ✔ / prod write **pending** | production write proof (PROD_QA gate); modelling gap — an experience about an EMPLOYER is stored as `subject_type='worker'` + profile id | **PARTIAL** | **yes** | author-vs-subject identity slice (organization subject) — code, no owner gate |
