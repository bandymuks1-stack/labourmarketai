...
| W6 | Trust / experience domain | **Author/subject schema ACTIVE in prod** (`20260806230000` applied 2026-08-06, ledger `20260806135649`); modelling gap CLOSED at schema level; local proof 35/35 + v1 43/43; **production WRITE proof IMPOSSIBLE today — no shipped surface**: `lib/trust/experience-eligibility.ts` is OWNER_DECISION_GATED ("no UI, no storage, no route") pending the §19 'Fit, ne reitingas' reconciliation | #972–#974 #977 #1008 #1037 #1042 | `20260802120000` + `20260806230000` APPLIED | local ✔ / prod write **blocked: no UI (owner gate)** | ship the W6 experience surface (submission + moderation queue + response) after the owner's §19 decision; then the PROD_QA write proof (identities + accepted booking `88a43ead` retained ready) | **PARTIAL — schema DONE, product surface NOT SHIPPED** | **yes** | owner §19 decision → W6 UI slice → production write proof |
| W7 | Employee journey | ~55%; zero P0; P1-1/#981, P1-2/#979 closed | #979 #981 | none own | local journey ✔ (83 shots) | P1-3 conversation memory (SQL still in `docs/proposals/`, never a migration); P1-4 `/dashboard/profile` **1007 lines** (verified) ~25 sections; P2-1 open-ended bookings skip the overlap guard | **PARTIAL** | **yes** | measure profile at 375px + section inventory (0 code — audit forbids a blind fix) |
