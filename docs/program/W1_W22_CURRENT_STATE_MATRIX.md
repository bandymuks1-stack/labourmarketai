...
| W6 | Trust / experience domain | **Schema ACTIVE in prod** (applied 2026-08-04, ledger 173→174); `experiences` result = `real`; local full cycle proven. **Author/subject slice CODE COMPLETE 2026-08-06**: org-scoped bookings resolve organization subjects, `author_side` model added, moderation queue names subject+author side; DB proof 35/35 + v1 regression 43/43 + rollback cycle + browser 9/9 (`docs/human-gates/experience-author-subject-v1-gate.md`) | #972–#974 #977 #1008 + author/subject PR #1037 | `20260802120000` APPLIED; `20260806230000` **APPLIED 2026-08-06 13:56 UTC (W6-D1), ledger 184→185, version `20260806135649`** | local ✔ / **prod write PROVEN 2026-08-06/07**: both QA submissions published via admin queue; org response `caf2e340` stored (0→1, `dce74d70` only) through the shipped form; duplicate refused live (`response_exists`); counts unchanged `{1,0,0,1}`; unrelated user 0 rows, anon 42501 (full proof: `W6_EXPERIENCE_SURFACE_CURRENT_TRUTH.md`) | none — production proof complete | **DONE (surface shipped + production proven)** | no | none — W6 closed; response moderation stays routine admin-queue work |
