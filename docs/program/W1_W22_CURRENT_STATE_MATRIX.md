...
| W7 | Employee journey | ~72%; zero P0; P1-1/#981, P1-2/#979 closed; **P1-4 inventory DONE**; **W7-S1 SHIPPED**; **W7-S3 SHIPPED** | #979 #981 #1051 #1052 | none own | local journey ✔ (83 shots) + profile measured at 1440/375 (`docs/audits/W7_PROFILE_FULL_INVENTORY.md`) + W7-S1 BEFORE/AFTER browser proof (`docs/audits/W7_S1_PROFILE_HUB_OVERVIEW.md`) + **W7-S3 six DOM fingerprints with ZERO product-visible differences + 21/21 interaction checks** (`docs/audits/W7_S3_PROFILE_READ_WATERFALL.md`) | **W7-S1 DONE** — five duplicate readiness summaries → ONE `ProfileHubOverview`; height −13.6…−19.0% desktop / −14.3…−19.1% mobile; cards 19→16; overview moved 4th block→1st; PROFILE simplicity 1.0→1.86 (PILOT_BLOCKER→NEEDS_POLISH). **W7-S3 DONE** — read waterfall 43→9 serial stages, 17→5 sequential DB round-trip stages, 24→0 serial translation awaits, 2→0 awaits inside the JSX; DB call volume and client bundle unchanged; behaviour-identical (fingerprint-proven). Local timing INCONCLUSIVE and stated as such — expected saving ≈108 ms at a 9 ms local RTT vs a 200–1200 ms dev-server noise band; worth ≈240–720 ms in production. REMAINING: W7-S2 a11y (34 unlabelled inputs, 70 sub-44px targets, all in `worker-availability-prefs-form`); W7-S4 two misplaced sections; W7-S5 non-worker identity copy; P1-3 conversation memory (SQL still in `docs/proposals/`, never a migration); P2-1 open-ended bookings skip the overlap guard | **PARTIAL** | **yes** | W7-S2 — label the 34 inputs and raise the 30–32px controls to 44px in `worker-availability-prefs-form.tsx` (one component, one guard, no migration) |
