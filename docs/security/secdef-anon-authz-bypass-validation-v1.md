...
| Recurrence | **OPEN** until a standing catalog guard exists. **It must be a bidirectional exact identity-signature ALLOWLIST, never a count.** Asserting "exactly 4 remain anon-executable" would pass if one reviewed public RPC disappeared in the same change that exposed one dangerous RPC — the total stays 4 and the guard lies. The guard must compare the **set** of anon-executable `SECURITY DEFINER` functions (keyed on `proname \|\| '(' \|\| pg_get_function_identity_arguments(oid) \|\| ')'`) against a reviewed allowlist, and fail on **both** an unexpected addition and the disappearance of an allowlisted entry. Spec: `secdef-public-execute-inventory-v1.md` §7 item 3; execution: `secdef-remaining-47-audit-plan-v1.md` §5. This defect already recurred across two independent migrations; without that assertion it will recur again. |
