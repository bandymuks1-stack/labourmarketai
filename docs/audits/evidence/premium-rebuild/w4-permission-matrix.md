...
| Preferred countries, bio, trust_score (`workers`) | FULL | **PARTIAL at DB, BLOCKED at render** — RLS exposes the whole row; no app surface selects these columns (finding M1) | BLOCKED | `0010_skills_and_worker_skills.sql:90` (broad table grant); `page.tsx:60-62` (8-column select) |
| `salary_min_eur` / `salary_max_eur` (`workers`) | FULL | **PARTIAL at DB, PARTIAL at render — rendered BY CONTRACT** — the scouting supply read selects `salary_min_eur` and the candidate card shows it as "rate from …". This is deliberate and consent-covered: pay expectation is one of the fields the discovery consent text promises (row 39), and it is on the owner-approved profile-safe allowlist. Corrected 2026-08-02 (W8 slice 1) — the previous wording said the app never selects these columns, which was false for the scouting surface | BLOCKED | `match-subject.ts:59` (select); `scout-safe-view.ts:86` → `worker-profile-visibility.ts:152` (`PROFILE_SAFE_PREVIEW_FIELDS`); `company/scouting/page.tsx:596` (render); `consent-definitions.ts:87-96` (consent text) |
...
| # | Finding | Disposition |
|---|---|---|
| M1 | `can_view_worker` gates the ROW, not columns: a consented employer session can select `workers.bio`, `trust_score`, `preferred_countries` via PostgREST even though no app surface renders them. Column-level privilege fix = migration. **Corrected 2026-08-02 (W8 slice 1):** `salary_min_eur` / `salary_max_eur` were listed here as unrendered and are not — the scouting card renders the minimum by product contract (see the dedicated row above), so for those two columns the gap is DB reach beyond the rendered value, not an unrendered field | **owner-gated item 10** (added to the W4 list) |
