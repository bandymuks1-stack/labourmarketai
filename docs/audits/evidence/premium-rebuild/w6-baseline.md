...
**3B — surfaces + proofs.** `/dashboard/experiences` (submitted-by-me in real state, published-about-me with the reply door and dispute door, count-only block, chat handoff) and `/dashboard/admin/experience-moderation` inside the EXISTING admin architecture (moderation queue + dispute queue as separate lists; only the canonical transition available from the current state; reason mandatory; decisions confirm their audit row). Both routes classified in the route truth map. `experience` i18n namespace ×12 locales (lt/en/ru/nl/de/da really translated — the i18n-debt ratchet did not move).
