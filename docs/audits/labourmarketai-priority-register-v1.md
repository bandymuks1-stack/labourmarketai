...
| **P1-12** | **No error monitoring of any kind** — no Sentry/OTel/Datadog; 136 `console.error` into Vercel logs; the one real error-recording design writes to `ai_runs`, which does not exist in prod. With P1-11, a broken signup would be invisible on three axes at once. | LOOP 7 T-10 | — | **125** |
