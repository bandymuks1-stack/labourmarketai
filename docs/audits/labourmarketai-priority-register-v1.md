...
|---|---|---|---|---|---|---|---|---|---|
| **P0-1** | **Anonymous authorization bypass in 7 `SECURITY DEFINER` RPCs.** `if v_owner <> auth.uid()` is NULL-unsafe; for `anon`, `auth.uid()` is NULL → NULL → treated as false → the raise never fires and the write runs with DEFINER privileges. Reachable because 54 of 205 secdef functions kept the default `PUBLIC` EXECUTE grant. | Live probe, rolled back: `set_marketplace_listing_status_v1` as `anon` → `NO_ERROR`, `status_now=closed`. Confirmed twice independently. LOOP 6 §sweep; LOOP 0 §3.3 | SEC | 5 | 3 | 5 | 5 | 2 | **187.5** — **RESOLVED, DB_PROVEN 2026-08-19.** Production now has **393** `SECURITY DEFINER` functions in `public`, of which exactly **8** are anon-executable, and every one is a deliberately public read/intake path: `search_public_vacancy_previews_v1`, `get_public_vacancy_preview_v1`, `count_public_vacancies_v1`, `list_public_vacancy_sitemap_v1`, `get_public_business_profile_v1`, `get_public_business_services_v1`, `get_public_business_listings_v1`, `submit_company_need_public_v1`. `set_marketplace_listing_status_v1` — the function the live probe exploited — is **no longer anon-executable**. Exactly **one** secdef function still contains a bare `<> auth.uid()` comparison (`conversation_counterpart_identities`), and it is authenticated-only, where `auth.uid()` cannot be NULL. The bypass class is closed. |
...
| P2-4 | 47 remaining anon-reachable secdef functions (3 with no authz logic at all). — **RESOLVED, DB_PROVEN 2026-08-19: 8 remain, all deliberately public (see P0-1).** | LOOP 6, PR #845 inventory |
