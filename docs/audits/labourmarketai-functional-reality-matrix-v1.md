...
| Worker documents centre | `/dashboard/documents` | **no write path anywhere.** Every `worker_documents` reference is a `select` (`lib/documents/document-centre.ts:46,116`; `readiness.ts:175`; `verified-cv.ts:227`; …). Only write on the page is `set_docs_aggregate_consent` (`consent-actions.ts:32`) | — | applied + **0 rows, and 0 is structurally unreachable** | `documents-readiness.test.ts`, `document-centre.test.ts` | **UI_ONLY** — a document inventory over a table nothing in the product can populate. The page says so (`documents/page.tsx:42-45,446`) |
...
| Save an opportunity (bookmark) | `/dashboard/opportunities` | `apps/web/lib/opportunities/saved-opportunities.ts:101` RPC `save_worker_opportunity_v1` | `worker_saved_opportunities` | applied + **0 rows** | `saved-compare-recent.test.ts` | **PARTIAL (never exercised)** |
...
|---|---|
| `/dashboard/documents` — worker document centre | Every reference to `worker_documents` in `apps/web` is a `select`. `apps/web/lib/documents/document-centre.ts:46` states "Read-only: no insert/update/delete/upsert, no storage access". Prod `worker_documents` = **0 rows** and 0 is unreachable — nothing in the product can create one. The only write on the page is the consent toggle (`consent-actions.ts:32`). The page itself discloses this (`documents/page.tsx:42-45`, note at `:446`). Downstream: `readiness.ts:175`, `verified-cv.ts:227` and `agency_pool_docs_readiness()` all compute "document readiness" over a permanently empty table. |
...
| 4 | `supabase/migrations/20260714150000_ai_runs_audit_v1.sql` | no `ai_runs` | **NOT APPLIED** | AI run audit log + persisted `AI_DAILY_RUN_BUDGET` counter (moot while `AI_PROVIDER_MODE` is not `live`) |
...
### F4 — The document centre is an inventory over a table nothing can write
