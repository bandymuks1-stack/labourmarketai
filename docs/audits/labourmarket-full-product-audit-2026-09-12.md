...
**Owner-gate batch (unchanged, request in parallel):** G-A supersede RPC migration (P0-3), G-B self-confirmation policy (EVID-2 / P0-4), G-C erasure path (#856 / P0-6), G-D K2-1 contact columns (PR #1430), G-E Supabase auth settings (leaked-password protection), plus the Gemini cost-basis confirmation, and `work_hour_allocations.source_reference` (per-document provenance on the organization ledger). G-F (constitution vs. worker stations) is **closed** by owner decision 0015: A-14 added, Work in Numbers declared as a distinct surface, ŠIANDIEN and PASAULIS stay on their existing routes as case A. None blocks the next slice.
