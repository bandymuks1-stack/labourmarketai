...
**Owner-gate batch (unchanged, request in parallel):** G-A supersede RPC migration (P0-3), G-B self-confirmation policy (EVID-2 / P0-4), G-C erasure path (#856 / P0-6), G-D K2-1 contact columns (PR #1430), G-E Supabase auth settings (leaked-password protection), plus the Gemini cost-basis confirmation, and — new from this walk — **G-F: amend Product Constitution A-01 to recognise worker stations (ŠIANDIEN · Work in Numbers · PASAULIS) as stable destinations, or record a scoped waiver for PR #1724**; until then the stations live on existing routes and `work_hour_allocations.source_reference` (per-document provenance on the organization ledger) waits with it. None blocks the next slice.
