-- ============================================================================
-- ROLLBACK for 20260727170000_null_safe_owner_guards_v1
--
-- Restores the EXACT pre-migration production definitions (verbatim from the
-- 2026-07-27 pg_get_functiondef dump, secdef-defs batch files) of the six
-- functions whose owner-equality guards were made null-safe.
--
-- WARNING: rolling back REINTRODUCES the I-02 NULL-flip authorization bypass
-- (dormant while no organizations row has owner_profile_id IS NULL). Apply
-- only via Supabase MCP after explicit owner approval.
--
-- CREATE OR REPLACE preserves existing ACLs — no grants are re-issued.
--
-- NOTE: the up-migration's `revoke execute ... from public, anon` statements
-- are deliberately NOT reversed here. They are an idempotent no-op on
-- production (anon/PUBLIC already hold no EXECUTE on these functions per the
-- 20260722160000 + 20260727120000 hardening) and re-granting anon/PUBLIC
-- would violate the standing secdef grant posture.
-- ============================================================================

begin;

-- 1/6 add_org_member — original production definition (batch-01.sql:466-491)
...
begin
  ...
  if not (public.is_admin() or v_owner = uid or public.manages_organization(p_org_id)) then return 'not_authorized'; end if;
...
-- 2/6 create_invitation_v1 — original production definition (batch-03.sql:282-387)
...
-- 3/6 get_team_capability_summary_v1 — original production definition (batch-04.sql:327-370)
...
-- 4/6 grant_org_manager — original production definition (batch-04.sql:459-487)
...
-- 5/6 respond_team_enquiry_v1 — original production definition (batch-07.sql:621-677)
...
-- 6/6 save_team_details_v1 — original production definition (batch-08.sql:554-653)
