-- ROLLBACK for 20260824130000_null_safe_owner_guards_v2.sql
-- Restores the six SECURITY DEFINER functions to their pre-fix bodies exactly
-- as captured from the live catalog (pg_get_functiondef, 2026-08-24) — the
-- ONLY difference from the forward migration is that each owner disjunct
-- reverts to the NULL-vulnerable `v_owner = uid` form. Signatures and ACLs are
-- unchanged (CREATE OR REPLACE). Applying this rollback re-opens the dormant
-- bypass; it exists only to satisfy the reversibility requirement.
