-- ROLLBACK for 20260831170000_timesheet_compute_allocations_v1.sql
--
-- Restores the timesheet_compute_lines_v1 body EXACTLY as shipped by
-- 20260819220000_timesheet_task_attribution_v1 — the allocation source
-- (`alloc` CTE, allocation lines, the allocation-wins dedupe and the combined
-- 500-line cap) simply disappears, and the doc-level 'source' returns to
-- 'journal_entry_metrics'. Extracted verbatim from that migration, not
-- retyped.
--
...
-- there is nothing a rollback could destroy. `work_hour_allocations` rows are
-- untouched — they simply stop being read by this function. Timesheet
-- snapshots already frozen in `timesheets.lines_snapshot` keep whatever
-- fields they were computed with — a rollback does not rewrite history, and
-- must not.
--
-- The signature is unchanged in both directions, so the SECDEF posture and
-- every existing revoke stay exactly as reviewed.
