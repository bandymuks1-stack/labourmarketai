...
-- has ONE shared answer to "when can you start, do you need housing, do you
-- bring your own transport" — today that answer has nowhere to live.
--
...
begin
  ...
  insert into public.team_details
      (org_id, availability_status, available_from, accommodation_needed,
       transport_own, max_trip_days, note, updated_at)
    values
...
revoke all on function public.save_team_details_v1(uuid, text, date, boolean, boolean, integer, text) from public;
grant execute on function public.save_team_details_v1(uuid, text, date, boolean, boolean, integer, text) to authenticated;
