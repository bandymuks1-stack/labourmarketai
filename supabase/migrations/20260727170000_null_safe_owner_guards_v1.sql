...
-- Never `db push`.
--
-- 20260727170000 — null_safe_owner_guards_v1
-- SECDEF authorization audit I-02 (2026-07-27): NULL-flip bypass in negated
-- owner-equality guards.
--
-- @human-gate-approved
--   Acknowledged RED because this file recreates SECURITY DEFINER functions
--   (and their bodies contain UPDATE/INSERT statements, which the static gate
--   flags as data-DML). The annotation makes CI pass; it does NOT authorise an
--   automatic merge or apply. Every change here TIGHTENS an authorization
--   guard — no privilege is granted, no guard is loosened, no schema changes.
--
-- ----------------------------------------------------------------------------
-- FINDING — I-02 NULL-flip authorization bypass class (audit 2026-07-27)
-- ----------------------------------------------------------------------------
-- `organizations.owner_profile_id` is NULLABLE with `on delete set null`
-- (supabase/migrations/0013_work_journal_m1.sql:35). Several SECURITY DEFINER
-- functions read it into a plpgsql variable and use it in a guard of the form
--
--   if not (public.is_admin() or v_owner = uid or manages_organization(...)) then
--     return 'not_authorized';
--   end if;
--
-- When v_owner IS NULL (org owner deleted, or never set):
--   v_owner = uid            → NULL
--   false OR NULL OR false   → NULL
--   NOT NULL                 → NULL
--   plpgsql IF NULL          → branch NOT taken → the deny is SKIPPED
--
-- i.e. ANY authenticated caller passes the guard for an owner-less org.
-- Currently DORMANT in production (0 organizations rows with NULL
-- owner_profile_id as of 2026-07-27) but structurally reachable at any time
-- via profile deletion (`on delete set null`).
--
-- Comparisons inside plain WHERE clauses (e.g. `where o.owner_profile_id =
-- auth.uid()`) and EXISTS-based guards are fail-closed and are NOT affected;
-- only the negated IF-guard variable comparisons are.
--
-- FIX (uniform, minimal): `v_owner = uid` → `(v_owner is not null and
-- v_owner = uid)` inside the affected guard. Nothing else in any body changes.
-- Each function below is the EXACT production definition (pg_get_functiondef
-- dump 2026-07-27, secdef-defs batch files) with ONLY that guard edited.
-- CREATE OR REPLACE preserves existing ACLs — no grants are re-issued.
--
-- Each CREATE is followed by an explicit
--   `revoke execute on function ... from public, anon;`
-- (required by the secdef-local-reset-reproducibility guard for every
-- SECURITY DEFINER function created after the 20260722160000 closure). On
-- production this is an idempotent NO-OP — anon/PUBLIC already hold no
-- EXECUTE on these functions (20260722160000 + 20260727120000 hardening) —
-- and on a fresh local reset it pins the same posture. `authenticated` and
-- `service_role` are never touched.
--
-- AFFECTED FUNCTIONS (6):
--   1. add_org_member                 (batch-01.sql:477)  — P1 fix
--   2. create_invitation_v1           (batch-03.sql:322)  — widened in sweep
--   3. get_team_capability_summary_v1 (batch-04.sql:350)  — widened in sweep
--   4. grant_org_manager              (batch-04.sql:471)  — widened in sweep
--   5. respond_team_enquiry_v1        (batch-07.sql:647)  — P1 fix (audit F-1)
--   6. save_team_details_v1           (batch-08.sql:576)  — P1 fix (audit F-2)
--
-- ROLLBACK: supabase/rollbacks/20260727170000_null_safe_owner_guards_v1.down.sql
--   (restores the exact current production definitions verbatim)
-- ============================================================================

begin;

-- ----------------------------------------------------------------------------
-- 1/6 add_org_member — I-02 (audit 2026-07-27, batch-01.sql:477)
-- Defect: `if not (is_admin() or v_owner = uid or manages_organization(...))`
--   where v_owner := organizations.owner_profile_id (nullable). NULL owner →
--   guard NULL → deny skipped → any authenticated user could attach any worker
--   as an active 'employee' engagement of an owner-less org.
-- Fix: null-safe owner comparison; everything else byte-identical.
-- (The separate consent-model question for this function stays
--  NEEDS_MANUAL_REVIEW — owner decision; unrelated to this guard fix.)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2/6 create_invitation_v1 — I-02 (audit 2026-07-27, batch-03.sql:322)
-- Defect: org-typed invitations guard `if not (is_admin() or v_org_owner = uid
--   or manages_organization(p_organization_id))` with v_org_owner :=
--   organizations.owner_profile_id (nullable). NULL owner → guard NULL → deny
--   skipped → any authenticated user could mint join_organization / join_team /
--   join_as_employee / collaborate_partner invitations for an owner-less org.
--   Found by the coordinator sweep that widened the fix set (the batch-03
--   audit table had marked this function CONFIRMED_SAFE — reclassified).
-- Fix: null-safe owner comparison; everything else byte-identical.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3/6 get_team_capability_summary_v1 — I-02 (audit 2026-07-27, batch-04.sql:350)
-- Defect: read guard `if not (is_admin() or v_owner = uid or
--   manages_organization(p_org_id))` with v_owner :=
--   organizations.owner_profile_id (nullable). NULL owner → guard NULL →
--   `return` (deny) skipped → any authenticated user could read the per-skill
--   member capability aggregates of an owner-less team org.
--   Widened into the fix set by the coordinator sweep (batch-04 table had
--   marked it CONFIRMED_SAFE — reclassified).
-- Fix: null-safe owner comparison; everything else byte-identical.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4/6 grant_org_manager — I-02 (audit 2026-07-27, batch-04.sql:471)
-- Defect: `if not (is_admin() or v_owner = uid) then return 'not_owner'` with
--   v_owner := organizations.owner_profile_id (nullable). NULL owner → guard
--   NULL → deny skipped → any authenticated user could grant an active
--   'manager' engagement (with operations_role) on an owner-less org —
--   privilege escalation, since manages_organization() then passes for the
--   grantee everywhere. Widened into the fix set by the coordinator sweep
--   (batch-04 table had marked it CONFIRMED_SAFE — reclassified).
-- Fix: null-safe owner comparison; everything else byte-identical.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5/6 respond_team_enquiry_v1 — I-02 / audit finding F-1, P1
--   (audit 2026-07-27, batch-07.sql:647)
-- Defect: `if not (v_team_owner = uid or manages_organization(e.team_org_id))`
--   with v_team_owner := organizations.owner_profile_id (nullable). NULL owner
--   → guard NULL → deny skipped → any authenticated user (other than the
--   sender) could accept/decline/expire a team enquiry addressed to an
--   owner-less team org (cross-tenant write to team_enquiries,
--   team_enquiry_events, audit_logs).
-- Fix: null-safe owner comparison; everything else byte-identical.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 6/6 save_team_details_v1 — I-02 / audit finding F-2, P1
--   (audit 2026-07-27, batch-08.sql:576-579)
-- Defect: `if not (is_admin() or v_owner = uid or
--   manages_organization(p_org_id))` with v_owner :=
--   organizations.owner_profile_id (nullable). NULL owner → guard NULL → deny
--   skipped → any authenticated user could create/overwrite the team_details
--   row (availability, deployable size, destination countries, note) of an
--   owner-less team org — surfaced wherever team availability is shown.
-- Fix: null-safe owner comparison; everything else byte-identical.
-- ----------------------------------------------------------------------------
...
--
--   select p.proname,
--          (pg_get_functiondef(p.oid) ~ 'is not null and') as null_safe_guard
--     from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--    where n.nspname = 'public'
--      and p.proname in ('add_org_member','create_invitation_v1',
--                        'get_team_capability_summary_v1','grant_org_manager',
--                        'respond_team_enquiry_v1','save_team_details_v1');
--
--   -- Expect: 6 rows, null_safe_guard = true for all.
--
--   -- Dormancy check (should stay 0; the fix matters exactly when it isn't):
--   select count(*) from public.organizations where owner_profile_id is null;
--
-- FUNCTIONAL CHECK: for every org with a non-null owner, behavior is
-- byte-identical (the added `is not null` conjunct only changes the outcome
-- when the owner column is NULL, where the old guard incorrectly allowed).
-- ============================================================================
