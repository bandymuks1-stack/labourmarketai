...
--
-- Invite-Ready Closure Train v1 — Wagon 0 (post-#840 review P1 repair).
--
-- Defect 1 (P1): `journal_entry_supersede` neither locked the old row nor
--   refused an already-superseded entry, and stamped `superseded_by`
--   unconditionally. Two concurrent saves of the same live entry could both
--   succeed, fork the chain into two visible descendants and overwrite the
--   first pointer. The #840 app-side stale read is not atomic.
--
-- Defect 2 (P1): the worker's explicitly SELECTED taxonomy skill evidence
--   (`worker_skills` + `journal_entry_skills`) was written app-side AFTER the
--   RPC committed; a transient failure was logged and swallowed while the UI
--   reported success and cleared dirty state.
--
-- Fix:
--   1. New `journal_entry_supersede_v2` — ONE transaction that locks the old
--      row (`FOR UPDATE`), verifies ownership / liveness / single-descendant,
--      validates every selected taxonomy slug server-side, creates the new
--      entry + metrics, carries forward decision markers and non-rederivable
--      skill links, upserts selected `worker_skills` (verified=false,
--      self-declared, never downgrades) + `journal_entry_skills` links, and
--      conditionally advances `superseded_by`. Any failed required write
--      raises and rolls back EVERYTHING — no partial live descendant.
--   2. Hardened legacy `journal_entry_supersede` (same signature) — row lock,
--      stale rejection (`entry_superseded`), conditional pointer update —
--      so the currently-deployed app is race-safe during rollout.
--
-- Structured errors (message text is the stable contract, matched app-side):
--   entry_not_found | not_owner | cannot_supersede_deleted | entry_superseded
--   | invalid_skill_slug | skill_slug_unknown
--
-- Honesty invariants: selected skills land as verified=false /
--   source='self_declared' / confidence_bin='yellow'; existing stronger rows
--   are NEVER downgraded (insert .. on conflict do nothing). No path creates
--   a verified skill claim.
--
-- Idempotent: CREATE OR REPLACE + fixed signatures. Reversible:
--   supabase/rollbacks/20260720100000_journal_atomic_supersede_v1.down.sql
-- ============================================================================

begin;

-- ── 1. Atomic v2: supersede + selected-skill evidence in one transaction ────
...
-- ── 2. Harden legacy journal_entry_supersede (rollout race safety) ──────────
-- Same signature and result; adds the row lock, stale rejection and
-- conditional pointer update so the currently-deployed app (which calls this
-- function until the new build ships) cannot fork the chain either.
...
-- ── 3. Harden journal_entry_restore (one-live-correction under restore) ─────
-- Review finding: restoring a soft-deleted correction could resurrect a
-- SECOND live correction of the same confirmed original (delete C → edit the
-- original again → restore C). Lock the row AND its correction parent, and
-- refuse the restore while another live correction of the same original
-- exists.
...
begin
  select worker_id, deleted_at, superseded_by, correction_of
    into v_worker_id, v_deleted_at, v_superseded_by, v_correction_of
    from public.journal_entries
    where id = p_entry_id
    for update;
  ...
  -- One live correction per original: serialize against concurrent
  -- supersedes of the parent (they lock the parent row too), then refuse if
  -- a competing live correction already exists.
  if v_correction_of is not null then
    ...
    if exists (
      select 1 from public.journal_entries c
       where c.correction_of = v_correction_of
         and c.id <> p_entry_id
         and c.deleted_at is null
         and c.superseded_by is null
    ) then
      raise exception 'correction_conflict_cannot_restore' using errcode = '42P10';
    end if;
  end if;
