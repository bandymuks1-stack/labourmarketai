...
- [ ] **PR #10b** — Journal security-hardening delta (`0014`) — **migration implemented** (`supabase/migrations/0014_journal_security_hardening.sql`, PR #18). Flag-lock, audit-on-confirm/reject/revoke, ledger `kind`/`reason`, locale CHECK, `proof_of_work` scaffold, direct-INSERT controls #1/#2/#5/#6/#7. *Pending DI DB-reset/RLS-test verification + production-migration approval.* **Automerge = NO.**
- [ ] **PR #10c (cutover)** — *follow-up to PR #18 (paired DB+app, separate PR)*: revoke the direct-INSERT policy on `journal_entry_confirmations`, cut `apps/web/lib/journal/confirm-actions.ts` (`confirmEntry`/`rejectEntry`) over to the `confirm_journal_entry`/`reject_journal_entry` RPCs, then remove the M1 compatibility path. Not required before PR #18 merge (audit fires on both paths today). *(Concern C from PR #18 review.)*
