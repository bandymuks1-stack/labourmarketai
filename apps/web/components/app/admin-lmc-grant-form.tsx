...
/**
 * Admin LMC credit form (commercial readiness audit v1). Credits internal
 * platform credit to a VERIFIED recipient through the existing ledger RPC.
 *
 * Honest states only: while `lmc_promotional_grants_enabled` is off the ledger
 * refuses the write and the form says exactly that — it never reports a credit
 * that did not happen. The idempotency key is prefilled and editable so a
 * retried grant is provably the same grant.
 */
export function AdminLmcGrantForm({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await adminGrantLmcAction({
        ...
        idempotencyKey: key.trim() || `lmc-grant:${campaign.trim()}:${email.trim()}`,
      });
      if (res.ok) {
  ...
  return (
    <div className="card-border flex flex-col gap-3 p-4" data-testid="admin-lmc-grant">
      {!enabled ? (
        <p
          className="rounded-md border border-state-amber/40 bg-state-amber/10 px-2 py-1 text-[11px] text-state-amber"
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <label className="flex w-24 flex-col gap-1 text-[11px] text-text-muted">
          {labels.amount}
          <input
            value={amount}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <label className="flex flex-[2] flex-col gap-1 text-[11px] text-text-muted">
          {labels.reason}
          <input
            value={reason}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {outcome?.kind === "ok" ? (
          <span className="font-mono text-[11px] text-state-success">
            {outcome.replay ? labels.replay : labels.granted}
          </span>
        ) : null}
        {outcome?.kind === "error" ? (
          <span className="font-mono text-[11px] text-state-warning">
