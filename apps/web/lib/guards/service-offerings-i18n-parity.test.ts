...
/**
 * W8 service_offerings — i18n parity guard (active locales lt/en/ru).
 *
 * `serviceOfferings` is an active-locale app namespace (like `mapLayers`). Every
 * leaf key must exist and be non-empty in all three active locales, so the
 * provider-facing services surface never renders a raw key or an English string
 * in LT/RU.
 */
...
const blocks = Object.fromEntries(
  ACTIVE.map((loc) => [
    loc,
    leafPaths((JSON.parse(read(`messages/${loc}.json`)) as { serviceOfferings?: unknown }).serviceOfferings ?? {}),
  ]),
) as Record<(typeof ACTIVE)[number], Map<string, string>>;

describe("serviceOfferings has full key parity across active locales", () => {
