...
/**
 * P0 marketplace — i18n parity guard (active locales lt/en/ru).
 *
 * `marketplace` is an active-locale app namespace. Every leaf key must exist and
 * be non-empty in all three active locales, so the discover/request surface never
 * renders a raw key or an English string in LT/RU.
 */
...
const blocks = Object.fromEntries(
  ACTIVE.map((loc) => [
    loc,
    leafPaths((JSON.parse(read(`messages/${loc}.json`)) as { marketplace?: unknown }).marketplace ?? {}),
  ]),
) as Record<(typeof ACTIVE)[number], Map<string, string>>;

describe("marketplace has full key parity across active locales", () => {
