...
describe("subscription uniqueness model (draft migration 20260721150000)", () => {
  ...
  const rollback = readFileSync(
    join(
      ...
      "supabase",
      "rollbacks",
      "20260806220000_stripe_multi_subject_v2.down.sql",
    ),
    "utf8",
  );
  ...
  it("the fresh package supersedes #844 — old migration is NOT in the tree", () => {
    expect(
      existsSync(join(REPO_ROOT, "supabase", "migrations", "20260721150000_stripe_subscriptions_v1.sql")),
    ).toBe(false);
  });
});
...
describe("org checkout fails closed until the linkage migration is applied", () => {
  it("the store returns needs-migration (never strips the org binding) on 42703", () => {
    ...
    expect(store).toMatch(/UNDEFINED_COLUMN\) return "needs-migration"/);
    expect(store).not.toMatch(/delete row\.organization_id/);
    expect(store).toMatch(/if \(u\.organizationId\) row\.organization_id = u\.organizationId/);
  });

  it("the checkout route binds the subject from the validated workspace", () => {
    const route = read("app/api/billing/test-checkout/route.ts");
    expect(route).toMatch(/resolveBillingSubject/);
    expect(route).toMatch(/planRequiresOrganization/);
    expect(route).toMatch(/checkoutMetadata/);
    expect(route).toMatch(/checkoutIdempotencyKey/);
    // the client can never supply an organization or price
    expect(route).toMatch(/z\.object\(\{ planKey/);
  });
});
