 ...
 * residual half re-landed 2026-09-01; supersedes the stale #1044 branch).
 *
 ...
 * The (b) half shipped earlier; this guard pins BOTH halves.
 *
 * Pinned here:
...
describe("company-verification review — P0 feedback guards", () => {
  ...
  it("an in-flight action is visibly marked (role=status pending indicator)", () => {
    expect(COMPONENT).toMatch(/company-verification-saving-/);
  ...
  it("all admin-page locales carry the saving label", () => {
    for (const loc of ["lt", "en", "de", "nl", "ru"]) {
      const messages = JSON.parse(
        ...
        admin?: {
          companyVerification?: { result?: { saving?: unknown } };
        };
      };
      expect(
