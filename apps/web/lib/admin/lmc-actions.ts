...
/**
 * Admin LMC credit action (commercial safe-prep v1; harvested from draft PR
 * #893 against current main).
 *
 ...
 * `lmc_promotional_grants_enabled` kill-switch (currently OFF — every call
 * today returns `grants_disabled`, honestly), the per-grant cap and the
 * mandatory expiry. This action only validates input shape and forwards it
...
const Schema = z.object({
  ...
  amountLmc: z
    .number()
    .positive()
    .max(LMC_ADMIN_GRANT_MAX_CENTS / LMC_CENTS_PER_UNIT),
...
export async function adminGrantLmcAction(
  ...
  // unauthorized caller should never reach the ledger at all. isSuperadmin
  // fails CLOSED on an unreadable signal — it never grants on an unknown.
