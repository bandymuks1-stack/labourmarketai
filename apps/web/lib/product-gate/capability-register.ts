...
const EDUCATION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    note:
      ...
      "AND ONE CORRECTION TO THIS ROW\'S OWN FIRST VERSION, which claimed the live programme\'s tile \'reads `demandUnknown`\'. IT DID NOT. `readInstitutionPrograms` mapped a profession missing from the count to `?? 0`, so the tile read \'0 live vacancies\' - a MEASURED ZERO for something nobody measured. The claim was aspirational and the code said otherwise; it was found in review on #1731 and fixed at the root (`demandCountFor`, a pure exported mapping with its own test). Absence is not zero for a structural reason worth keeping: `count_public_vacancies_by_profession_v1` is a TOP-N read (`limit 20` by default) over the live public pool, so a profession is absent when it is outside the window exactly as much as when it has no vacancy, and the rows cannot tell those apart. Re-measured 2026-09-13: 20 professions come back, not the 39 recorded on 2026-09-08 - a live pool moves - and `builder`, the one live programme\'s direction, is not among them. NOW the tile reads `demandUnknown` and the export writes `unknown`. SEP-7 is held by the reader, not only by the surface.",
  },
];
