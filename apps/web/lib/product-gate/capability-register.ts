...
const PERSON: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    surfaces: ["app/[locale]/dashboard/profile/page.tsx"],
    note: "BLOCKED → PARTIAL, 2026-09-15. The owner decision this row carried was RESOLVED by #1740: the split `external_profiles_v1` was reviewed, approved and APPLIED as ledger `20260915042406`. Verified on production 2026-09-15: `worker_external_profiles` exists with `worker_external_profiles_select` and the two commands `save_worker_external_profile_v1` / `disconnect_external_profile_v1`; 0 rows. PARTIAL on volume — no person has linked a profile yet — and the section on /dashboard/profile is live rather than 'not enabled yet'.",
  },
  {
    id: "PER-12",
    domain: "person",
...
const SKILLS: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    strongestEvidence: "TEST_PROVEN",
    anchors: ["lib/skills/recognition-model.ts"],
    coreModule: "lib/skills/recognition-model.ts",
    surfaces: [],
    note: "MISSING → ARCHITECTURE_ONLY, 2026-09-15. The OBJECT MODEL exists and is tested: `lib/skills/recognition-model.ts` names the five states of SEP-6 as a closed set, derives a person's standing from what already exists (confirmed journal evidence → DEMONSTRATED; a current credential → VALID; a current assessor recognition → RECOGNISED EQUIVALENCE; nothing → MISSING; `formal_qualification` is never emitted because nothing can evidence it), and states the one authority rule — an assessor-capable organization (`training_provider`), managed by the actor, that is neither the subject nor an organization engaging the subject as a worker. ESCO is the semantic layer only; nothing scores. What is deliberately NOT built without the owner: the RECORD of an assessor's act. Its packet is prepared and unapplied — `20260915140000_competency_recognitions_v1`: one relation (subject, requirement kind/key/country, evidence entry ids snapshotted, assessor organization and person, decision, validity, revocation with reason, correction by superseding row; readable by the subject always and by the assessing organization's managers, never by employers directly) and two definer commands that enforce the authority rule and admit only CONFIRMED evidence of the subject's own. Dry-run on production in a rolled-back transaction, ten stages: employer refused not_an_assessor_role; stranger refused actor_does_not_manage_assessor; the subject (who owns a training provider) refused self_recognition; the admin of an organization that engages the subject refused beneficiary_organization; a non-confirmed entry refused 22023; the independent institution recorded on confirmed evidence and read 1; the subject read 1; the employer read 0 and could not revoke; revocation without a reason refused, with a reason recorded; anon refused; zero objects left behind. No surface yet — a surface for an unapplied authority object would be a control that returns 42501 to the one person it exists for.",
    ownerDecision:
      "SKL-9 / ARCH-2: apply `20260915140000_competency_recognitions_v1` (independent-assessor RPL record; legally significant authority) via MCP `apply_migration`, or decline — and confirm `training_provider` as the assessor-capable role for now.",
  },
  {
    id: "SKL-10",
    domain: "skills",
...
const WORK_EXECUTION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    status: "PARTIAL",
    strongestEvidence: "TEST_PROVEN",
    anchors: ["lib/projects/team-assignment.ts", "lib/projects/team-assignment-actions.ts", "components/app/team-assign-form.tsx"],
    coreModule: "lib/projects/team-assignment.ts",
    surfaces: ["app/[locale]/dashboard/company"],
    note: "MISSING → PARTIAL, 2026-09-15. A whole team can now be assigned to a project from the team panel on /dashboard/company: `assignTeamToProject` fans the brigade out through the ONE existing per-person write, `assign_worker_to_project`, so every member is admitted or refused by the database's own two gates (the caller manages THIS project; this person is on the caller's roster or holds a booking engagement for this exact project) and each outcome is shown by name. It invents no authority and is deliberately not atomic — a partial brigade is a true outcome the manager must see. Worker freedom is kept structurally: after the writes, every assigned member's calendar is read ONCE and each gets their OWN reservation verdict for the project's window, shown beside their name; a clash warns and never undoes (SEP-2), an unread source is unknown and never clear (SEP-7). WHAT IS STILL MISSING, and why this is PARTIAL rather than usable: the record that these people were assigned AS A UNIT. No team↔project link exists, so nothing can end the brigade's assignment as one act, and a brigade match (DEM-6) has nothing to point at. That link is a schema change — owner-gated — and is the remaining half. Production holds 0 teams, so nothing is walkable regardless; the previous note's measurement of the team layer (create_team_v1, team_details, team_enquiries, join_team invitations all EXIST and are usable, usage 0) still stands.",
    ownerDecision:
      "J-COMPANY-EXECUTION step 4, the UNIT half: apply `20260915130000_project_team_assignments_v1` (team↔project link riding the existing per-person writes; dry-run proven on production) via MCP `apply_migration`, or decline.",
  },
  {
    id: "WRK-7",
    domain: "work_execution",
...
const TIME_CAPACITY: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    ownerDecision:
      "J-TIME-FREEDOM step 5: apply `20260915120000_commitment_override_receipts_v1` (append-only override receipt readable by the worker it concerns; dry-run proven on production) via MCP `apply_migration`, or decline.",
  },
  {
    id: "CAL-8",
    domain: "time_capacity",
