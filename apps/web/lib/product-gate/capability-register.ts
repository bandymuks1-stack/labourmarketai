...
const DEMAND_SUPPLY: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    note:
      "Owner-approved and APPLIED 2026-09-07 (ledger 20260907180546), then called on the LIVE function under three real users' auth: a manager of two organizations saw 2 of 2 supply rows, the agency that authored one saw 1 of 2, someone who manages nothing saw 0 with no error, and anon is refused 42501 at the privilege level. The supply side of the market is readable for the first time. PARTIAL for a measured reason, now traced to its cause. All three agency_offer rows on production carry NULL role_or_work_type, country and team_size, so an employer discovers THAT capacity exists without learning its SHAPE. The information is not missing - it is in the wrong place. lib/demand/demand-request.ts validates workType, country, teamSize and startPeriod against closed sets and writes them to the structured columns in a follow-up UPDATE, but agency supply never goes through it: lib/demand/demand-drafts.ts gives agency_offer its own FREE-TEXT payload shape (candidateRoles, skillAreas, countries, availability, documentation) and nothing maps those into the structured columns. One production draft holds candidateRoles 'Stalius', skillAreas 'Statyba', countries 'No' and documentation 'A1' - real, useful shape sitting in a column the gated read deliberately never projects. Widening the read to payload is the WRONG fix and the migration explicitly refuses it: payload is free text with no closed set, and this read exposes six non-identifying columns by design. The next link is the DECLARATION path - normalise the agency offer into the same validated closed-set columns the company request already populates. SEP-8 in one row: the data exists and is not reachable.",
  },
];
