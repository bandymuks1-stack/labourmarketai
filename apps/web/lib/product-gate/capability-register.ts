...
const EDUCATION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    note:
      ...
      "WHERE THE REMAINING UNKNOWN IS. The demand read is PRODUCTION_DATA_PATH_PROVEN; the EXPORT is TEST_PROVEN only - it is new, no human has downloaded it and it has not been run against production. Also re-measured 2026-09-13: `count_public_vacancies_by_profession_v1` returns 20 professions today, not the 39 recorded on 2026-09-08 - it counts a LIVE pool and that number moves - and `builder` is not among them, so the live programme\'s demand tile reads `demandUnknown`. That is an honest UNKNOWN under SEP-7, not a defect and not a zero.",
  },
];
