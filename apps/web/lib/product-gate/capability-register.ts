...
const ORGANIZATION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    surfaces: ["app/[locale]/dashboard/company"],
    note: "`agency_client_connections` is live. `agency_clients` was APPLIED 2026-09-14 (owner item 4d, ledger `20260914202322`) under the confirmed canonical Model B: its policy is `owns_company(company_id) OR is_admin()` - company/org authority, never the legacy `owns_agency` - so applying it did not revive Model A (see ORG-10). The two are NOT rivals and this row used to imply they were: `agency_client_connections` is an invitation-based bidirectional bridge to a real platform organization, `agency_clients` is an agency's own private record of a client who may not be on the platform. `AgencyClientsSection` on /dashboard/company had been degrading against a 42P01 since it shipped; read back under a REAL staffing-agency owner's auth after the apply, the relation now reads cleanly (0 rows). 0 rows is adoption, not breakage - 4 staffing-agency companies can now use it. The demand link is one additive nullable column, `customer_requests.agency_client_id`; `customer_requests` was re-counted at 20 rows after the apply, unchanged, 0 linked. DELIVERY + COMMUNICATION (2026-09-24): `create_agency_client_connection_v1` only INSERTED a row keyed on the client's e-mail — nothing ever reached that e-mail, so step 1 of the owner's acceptance walk ended in a row nobody could see, and no contact permission existed for an active connection, so step 10 (agency ↔ client message) was impossible. The invite action now ALSO creates an `invite_company` invitation (marked `proposed_role = agency_client`) through the ONE invitation primitive (lib/invitations) and hands the agency the token link; e-mail stays inert until INVITE_EMAIL_* exists and the section says `created` (link ready), never `sent`. The invite page names a masked addressee when the signed-in e-mail differs (the connection accept RPC still requires e-mail equality — its widening is a RED owner item). `allowed_agency_connection` (lib/agency/bridge-conversation.ts) opens the conversation between the two consenting people once the connection is ACTIVE. Still PARTIAL: not yet walked in a browser by two real companies since the change.",
  },
  {
    id: "ORG-10",
  ...
  },
  {
    ...
    note:
      "ONE distribution primitive over the canonical token `invitations` system (ORG-3), extended in place by 20260917120000: an open shareable link (no addressee), a bounded multi-use campaign link (1..500 seats, each acceptance its own person and its own `invitation_acceptances` ledger row), an employer → person invitation to a canonical need (`invite_to_demand` → customer_requests; acceptance writes a demand_interest_signals row whose snapshot names `employer_invitation` and carries no match band), and an approved-external-source referral door (`/api/referrals/external/v1`, one machine secret per registered source, consent re-checked in the database, idempotent on (source, reference), no worker table of its own). The logged-out landing shows a minimal preview and both doors (register / sign in) with the safe `?next=` return; the person who accepts an external referral reviews each declared line (accept / reject / correct) and NOTHING is written to their skills or professions by any of it. 20260917120000 was APPLIED to production 2026-09-17 (ledger 20260917080303) and the one-identifier forward correction 20260917130000 was APPLIED the same day (ledger 20260917091045); every RPC — including the corrected signed-in demand preview — was exercised on production as the real roles in zero-residue transactions (PRODUCTION_RPC_PROVEN, identity counters unchanged). `EXTERNAL_REFERRAL_TOKEN_NONSTOP` is set in Vercel Production. CORRECTED 2026-09-19 (window 4): #1752 was MERGED 2026-09-17 09:42 UTC (squash `4ff163da`) and has been on production since — the owner decision this row carried (\"merge #1752\") was already resolved and had gone stale, exactly the class the 2026-09-07 pass found; production holds 3 invitations minted after that merge and 1 acceptance-ledger row. What remains is NOT an owner decision: the browser cases (CASE 1–19) and the live partner-door proofs are walked on production, after which this row moves to BUILT_AND_USABLE / HUMAN_UI_PROVEN. PARTIAL until then, because the code is reachable and proven at the RPC level while no human has driven the new doors end to end.",
  },
];
