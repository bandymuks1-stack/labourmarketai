...
const EDUCATION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    surfaces: ["app/[locale]/dashboard/company"],
    note:
      "THE 2026-09-08 GAP IS CLOSED, and this note said otherwise for five days. It read: a programme cannot be CORRECTED, there is no update function in `pg_proc`, a field skipped once is skipped permanently, and the one live programme reads `demandUnknown` and always will. Re-measured against production 2026-09-13, every clause of that is now false. `update_education_program_v1(uuid, text, text, text, text)` EXISTS as a SECURITY DEFINER function (migration 20260908120000, PR #1648), so a programme is correctable; and the single production programme now CARRIES a target profession - `builder`, vocational, description still empty. " +
      "WHAT IS ACTUALLY TRUE ON 2026-09-13, and it is a different fact: the institution's demand tile for that programme still shows no number, because `count_public_vacancies_by_profession_v1()` returns 20 professions and `builder` is NOT one of them. That is an ABSENT ROW in the public vacancy pool, not a missing target and not a broken reader - the surface degrades to `demandUnknown`, which is the correct answer (SEP-7: UNKNOWN is not ZERO). Nothing in the education vertical has to change to fix it; supply of public vacancies for that profession does. " +
      "The member gap is unchanged and is still the real one: one programme, one cohort, ZERO members. Every write path exists and is reachable on `/dashboard/company` - create programme, correct programme, create cohort, assign learner, remove member - and production holds 1 accepted `student` invitation, so the assignable list is not empty. What is missing is a human doing it, not a control to do it with.",
  },
  {
    id: "EDU-3",
    domain: "education",
