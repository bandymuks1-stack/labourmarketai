...
const WORK_EXECUTION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    note:
      "RECLASSIFIED 2026-09-14, re-measured against the code and the live database. The 2026-09-08 note called this “genuinely unreachable” on one criterion — no surfaceRoute of its own in the dashboard module registry — but BUILT_NOT_CONNECTED means “no product path leads to it”, and a panel mounted on a reachable page IS a product path. MEASURED: `ProjectDefectsPanel` is mounted on the project operations page, which is linked from the project map, the company home field, the workspace project result and the assignment manager; `defects` and `defect_corrections` are BOTH APPLIED in production; the panel carries four write actions and honest pre-apply and empty states. THE REAL LIMIT, and it is a real one: the operations board is MANAGER-ONLY by explicit decision (F11), so a worker cannot see — or answer — a defect raised against their own work. A quality loop with only one side of the conversation is half a loop. Production holds 0 defects, so nobody has used it either way.",
  },
  {
    id: "WRK-9",
    domain: "work_execution",
...
const MARKETPLACE: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    surfaces: ["app/[locale]/dashboard/finance"],
    note:
      "RECLASSIFIED 2026-09-14, re-measured against the code and the live database. The 2026-09-08 note called this “genuinely unreachable” on one criterion — no surfaceRoute of its own in the dashboard module registry — but BUILT_NOT_CONNECTED means “no product path leads to it”, and a panel mounted on a reachable page IS a product path. MEASURED: `TripsSection` is mounted on the finance page, `business_trips` and `business_trip_events` are APPLIED in production, and the section wires the full lifecycle — create, update, submit, decide, complete, cancel. THE REAL LIMIT is exactly what the old note’s headline said, and it is still true: a trip NEVER REACHES THE CALENDAR. `PLANNING_SOURCE_TYPES` in `lib/planning/planning-model.ts` lists booking, project, task, journal, finance, invitation, absence and stage — there is no `trip`. So a person can be sent somewhere for three days and the plan that decides whether they are free will not know. Production holds 0 trips.",
  },
  {
    id: "MKT-7",
    domain: "marketplace",
