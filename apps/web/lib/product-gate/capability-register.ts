...
const DEMAND_SUPPLY: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    surfaces: ["app/[locale]/dashboard/opportunities"],
    note: "BUILT, NOT YET APPLIED TO PRODUCTION. The only item in the 2026-09-14 wave that genuinely needed new storage, and the owner authorized exactly that (E5). `worker_saved_opportunities` remembers ONE opportunity already found; nothing remembered the QUESTION, so a worker re-asked it by hand and heard about new work only by happening to look. `supabase/migrations/20260914140000_worker_saved_searches_v1.sql` adds ONE worker-owned table with three gated RPCs, shaped on the saved-opportunities precedent: private by RLS (an employer never learns who is looking for what), RPC-only writes, 20 per worker, and a criteria column the DATABASE keeps closed to the seven discovery dimensions. No demand facts are copied, so a saved search cannot go stale. NO second notification path and no second matcher: the alert is one more `notification_events` type written by the ONE audited emitter honouring the SAME preferences, and the matching is `applyDiscoveryFilters` — the board’s own — run over the cards the worker’s own read returned, so the count and the results cannot disagree. POINTER-ONLY like the weekly digest, exactly-once per search per ISO week through a deterministic entity id whose implementation was DE-DUPLICATED out of the digest emitter rather than copied. SEP-7: `newSinceSeen` is null, never zero, when a match carries no date, and an unknown count can never raise an alert. Proven on a real PostgreSQL 16 server — `scripts/db-proof/worker-saved-searches.sh`, 26/26 — which caught two defects before they could reach production: a CHECK containing a subquery (illegal in PostgreSQL, would have failed the apply) and a rollback whose data guard printed its refusal and then dropped the table anyway. RED class: merged behind the human gate, production apply is a separate owner act, so the board renders no saved-search control until then (honest invisibility, never a dead button). PARTIAL until applied, and because delivery reaches a worker who opens the board — reaching those who do not is the email half, deliberately owner-gated behind notification consent.",
  },
  {
    id: "DEM-9",
    domain: "demand_supply",
...
const MARKETPLACE: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    surfaces: ["app/[locale]/dashboard/company"],
    note: "FIX WRITTEN AND PROVEN, NOT YET APPLIED TO PRODUCTION. The defect (CONFIRMED against the live function body 2026-09-14): `issue_asset_v1` checked authority, required a target and validated the condition enum, then inserted an `issued` assignment and set `assets.availability = assigned` WITHOUT testing current availability, without checking for an existing open assignment, and without `for update` — so two managers could issue the same physical asset twice, and `lib/assets/assets.ts` kept only the FIRST open assignment per asset, hiding the second from the very screen that should have shown it. `supabase/migrations/20260914120000_asset_single_open_assignment_v1.sql` (owner-approved 2026-09-14) adds a partial unique index on one open assignment per asset and takes `select … for update` on the asset row in issue/transfer/return before any decision. Measured on a real PostgreSQL 16 cluster with two genuinely concurrent sessions (`scripts/db-proof/asset-single-open-assignment.sh`, 22/22): 2 open assignments before, 1 after, the loser blocking 2046ms on the row lock and then refused in words a person can act on. RED class (SECURITY DEFINER replacement): merged behind the human gate, production apply is a separate owner act, so PRODUCTION STILL RUNS THE OLD FUNCTION. Production exposure meanwhile: 0 assets, 0 asset_assignments. Stays PARTIAL until the migration is applied.",
