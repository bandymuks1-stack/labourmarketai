...
const ORGANIZATION: readonly CapabilityRow[] = [
  ...
  },
  {
    ...
    note:
      "ONE distribution primitive over the canonical token `invitations` system (ORG-3), extended in place by 20260917120000: an open shareable link (no addressee), a bounded multi-use campaign link (1..500 seats, each acceptance its own person and its own `invitation_acceptances` ledger row), an employer → person invitation to a canonical need (`invite_to_demand` → customer_requests; acceptance writes a demand_interest_signals row whose snapshot names `employer_invitation` and carries no match band), and an approved-external-source referral door (`/api/referrals/external/v1`, one machine secret per registered source, consent re-checked in the database, idempotent on (source, reference), no worker table of its own). The logged-out landing shows a minimal preview and both doors (register / sign in) with the safe `?next=` return; the person who accepts an external referral reviews each declared line (accept / reject / correct) and NOTHING is written to their skills or professions by any of it. 20260917120000 was APPLIED to production 2026-09-17 (ledger 20260917080303) under owner authorization, and every RPC was then exercised on production as the real roles in a zero-residue transaction (20 cases, identity counters unchanged) — PRODUCTION_RPC_PROVEN. `EXTERNAL_REFERRAL_TOKEN_NONSTOP` is set in Vercel Production. What is NOT live: the #1752 code itself (production runs main, the partner endpoint is a 404 until merge), and the one-identifier forward correction 20260917130000 (`get_invitation_preview_v2` read `role_text`; the column is `role_or_work_type`) — RED, PREPARED, NOT APPLIED — without which a demand invitation's signed-in landing renders the generic load error.",
    ownerDecision:
      "Apply the forward correction 20260917130000_invitation_preview_demand_column_fix_v1 (one identifier in one SECURITY DEFINER function; RED by class) and merge #1752 so the code reaches production; the browser cases (CASE 1–19) are then walked on production.",
  },
];
