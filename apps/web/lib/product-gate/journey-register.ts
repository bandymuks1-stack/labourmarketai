  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        link: "LIVE",
        proof:
          "PRODUCTION_DATA_PATH_PROVEN. THIS ENTRY WAS STALE, AND IN THE DANGEROUS DIRECTION: it described the state on 2026-09-08 and was never re-measured after the gate it asked for was closed. Its reason said `organization_evidence_events` has exactly two INSERT policies and that none admits a subject, so a refuse button would return 42501 to the one person it exists for. That was true when written and false a week later. BOTH HALVES NOW EXIST. SEEING: the subject's branch of `organization_evidence_parties_select` was restored by ledger `20260908080950`, which lifts the recursing predicate into the SECURITY DEFINER boolean `is_evidence_record_subject` (returns no rows, leaks no column, widens no policy); the section is mounted on `/dashboard/profile`. REFUSING: the write path the old reason correctly sized as RED was built and APPLIED — `20260915180000_subject_contest_and_clash_receipt.sql` as production ledger `20260915185038` (note the version drift the ledger records), adding a THIRD INSERT policy `organization_evidence_events_subject_dispute`, admitting only `event_type = 'disputed'` from the linked subject and forcing `actor_role IS NULL`. Runtime-proven BEFORE the apply against a throwaway PostgreSQL 16.13 with RLS on, every subject-facing case executed as the non-owner role `authenticated`: 43 assertions, 43 pass, rollback included (`docs/db-proof/RED-4-5-runtime-proof.md`). Post-apply readback measured as a DELTA: policies 348 -> 349, the fingerprint of all OTHER policies identical. The surface is `disputeEvidenceRecordAction` + `components/app/organization-evidence-section.tsx`, and `lib/guards/subject-authority-surfaces.test.ts` pins the honesty of it: a dispute is an event ALONGSIDE the record, it cannot edit or delete the organisation's text (that table has no UPDATE and no DELETE policy), and UNKNOWN stays UNKNOWN. LIVE describes the CHAIN, exactly as the sibling step above it does; EVID-1 stays PARTIAL because no human has yet completed an import, so no subject has yet had anything to refuse.",
      },
    ],
  },
  {
