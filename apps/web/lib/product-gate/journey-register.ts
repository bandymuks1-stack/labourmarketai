  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        link: "BROKEN",
        because:
          "Half of this link is live, so NOT_BUILT was wrong — and the old reason ('outcomes have no writer a human can reach') was itself corrected on the capability side on 2026-09-08 and never here. Outcomes (EDU-3) need no writer by construction: there is no outcomes TABLE, only an aggregate that DERIVES counts over the institution's existing active `student` contexts, and it has two real consumers — the institution learners section and the chat education answers. Proven live under real auth: a manager of a training_provider organization got learners=1 with suppressed=true, the k-anonymity floor of 5 nulling the four counts so a number can never identify one person; a non-manager was REFUSED 42501. The OTHER half was described as absent (EDU-6, MISSING: 'an institution cannot see employer demand'), and re-measuring it on 2026-09-08 showed that was REDDER than the product. The demand read is BUILT, REACHABLE and CORRECT: `readInstitutionPrograms` composes `count_public_vacancies_by_profession_v1` - live, 39 professions with real counts, EXECUTE to `authenticated` and not to `anon` - and `institution-programs-section.tsx` renders it per programme at `program-demand-<id>` on `/dashboard/company`, degrading to `demandUnknown` rather than to 0. The step stays BROKEN for two narrower reasons. There is still NO REPORT AND NO EXPORT, so the institution can read demand and not take it anywhere. THE SECOND REASON IS RETIRED, re-measured against production 2026-09-13: it said no update function exists, a programme is immutable after creation, production's single programme has no target profession, and its demand number is `demandUnknown` permanently - and every clause of that is now false. `update_education_program_v1(uuid, text, text, text, text)` exists as a SECURITY DEFINER function (migration 20260908120000, PR #1648) and the single programme now carries `builder` / vocational. Its demand tile still shows no number for a DIFFERENT and legitimate reason: `count_public_vacancies_by_profession_v1()` returns 20 professions and `builder` is not among them, so the surface degrades to `demandUnknown` - an absent row in the public vacancy pool, which is the honest answer (SEP-7), not a defect to fix in the education vertical. What is left of this link is therefore ONE thing: the institution can read employer demand and cannot take it anywhere. A report or an export is what unblocks it.",
      },
    ],
  },
  {
