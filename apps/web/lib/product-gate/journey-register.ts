    ...
    steps: [
      ...
      },
      {
        ...
        proof:
          ...
          "REASON TWO - 'the signal is inert in practice ... NO update function exists ... a programme is immutable after creation, production's single programme has no target profession, and its demand number is `demandUnknown` permanently' - was FALSE WHEN WRITTEN and this register repeated it for five days. Read-only SQL against production 2026-09-13 contradicts every clause: `update_education_program_v1(uuid,text,text,text,text)` exists (SECURITY DEFINER, migration `20260908120000`), `EditProgramForm` is mounted beside the create form, and the live programme carries `builder` / `vocational` - it HAS been corrected. Its demand tile still shows no number, for a DIFFERENT and legitimate reason: `count_public_vacancies_by_profession_v1` is a TOP-N read (`limit 20`) over a LIVE pool, returns 20 professions today, and `builder` is not among them. A profession outside that window is indistinguishable from one with no vacancy, so absence is UNKNOWN (SEP-7), not a break - and not a zero. THAT LAST WORD WAS WRONG WHEN THIS STEP FIRST WENT LIVE: the reader mapped a missing profession to `?? 0`, so the tile in fact read \'0 live vacancies\'. Found in review on #1731 and fixed at the root in `demandCountFor`, which is now a pure exported mapping with its own test - the previous tests had constructed the null by hand and so could not have caught it. " +
