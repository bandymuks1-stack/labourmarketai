...
export const JOURNEY_REGISTER: readonly Journey[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        link: "BROKEN",
        because:
          "NOT_BUILT → BROKEN, 2026-09-15: 'exists at no layer' is no longer true and 'recognised' is still not. The MODEL layer exists (SKL-9 ARCHITECTURE_ONLY, TEST_PROVEN): the five states of SEP-6 as a closed set, a derivation that turns confirmed real work into DEMONSTRATED (never RECOGNISED, never VALID), and the one authority rule — an independent, assessor-capable organization, managed by the actor, that is neither the subject nor anyone engaging the subject. The RECORD layer — an assessor's act — is a legally significant authority object and is owner-gated: `20260915140000_competency_recognitions_v1` is prepared, dry-run on production through ten authority stages in a rolled-back transaction, and unapplied. Until the owner applies it, 'five years of real work' still reads as 'certificate missing' on every surface, because the surface that would say 'demonstrated — could be assessed' would point at a record nobody can create. Owner decision on SKL-9.",
      },
    ],
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        link: "BROKEN",
        because:
          "NOT_BUILT → BROKEN, 2026-09-15: the ASSIGNING now exists and the UNIT does not. From the team panel a manager assigns the whole brigade to a project they manage; each member goes through the ONE existing per-person write and the database's own gates, each outcome is named, and each assigned member's calendar gets its own verdict for the project's window — so a brigade assignment cannot silently put a person in two places (WRK-6 PARTIAL, TEST_PROVEN). What does not exist is the record that they were assigned AS A UNIT: no team↔project link, so the brigade cannot be ended as one act and a brigade match has nothing to point at. That link is new schema — RED, owner-gated — and its packet is PREPARED and UNAPPLIED: `20260915130000_project_team_assignments_v1` (the unit row + which members it gave; `assign_team_to_project_v1` riding the existing per-person write, `end_team_project_assignment_v1` taking back only what the unit gave; SELECT for the project's managers, the team's owner and the team's MEMBERS). Dry-run on production in a rolled-back transaction through the real `create_team_v1` and `add_org_member`: unit assigned, member assigned, idempotent, the member sees the unit, an outsider reads 0 and is refused to end it, the owner's end returns the member's assignment, anon refused; zero objects left behind. Owner decision: apply it via MCP `apply_migration`, or decline. Production holds 0 teams (a human fact, not a code gap), so no walk is possible until someone creates one.",
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        link: "BROKEN",
        because:
          "Corrected 2026-09-14 and again 2026-09-15. SKL-10, the training and certification register, is APPLIED and PARTIAL and deliberately writes nothing into the skill ladder (a certificate is not a demonstrated competency — SEP-6). SKL-9 is no longer 'genuinely MISSING': its object model exists and is tested, and the recognition RECORD — the assessor's act — is prepared as `20260915140000_competency_recognitions_v1`, dry-run on production and unapplied. For the INSTITUTION this is the exact chain the owner named: the institution IS the `training_provider`, so once the record is applied a learner's confirmed practice can be recognised by that institution against a formal requirement — while the same institution is refused as assessor for anyone it engages as a worker (beneficiary rule, proven in the dry-run). The chain waits on one owner decision (SKL-9), not on engineering.",
      },
      {
