...
describe("conversation layer performs no direct data write", () => {
  ...
  for (const file of CONVERSATION_FILES) {
    ...
    it(`${rel} contains no direct insert/update/delete/upsert/rpc`, () => {
      ...
      // Direct table mutations and RPC calls are forbidden in this layer.
      expect(src, rel).not.toMatch(/\.(insert|update|delete|upsert)\s*\(/);
      expect(src, rel).not.toMatch(/\.rpc\s*\(/);
      // The pure registry module must not even touch the DB client.
      if (rel.startsWith("lib/conversation/")) {
